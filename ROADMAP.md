...
- `migrations/2026-07-15_chat.sql` — adds `chat_threads` + `chat_messages` (Coach Chat), with RLS + `service_role_bypass` matching the other 11 tables. **Not yet run** — deliver-for-manual-run, per the 2026-07-15 Coach Chat changelog entry above.
- `migrations/2026-07-15_chat_proposals.sql` — adds `chat_proposals` (Coach Chat tool-use write proposals), RLS + `service_role_bypass`. **Not yet run** — deliver-for-manual-run.
- `migrations/2026-07-15_profile_timezone.sql` — adds `profiles.timezone` (text, IANA identifier, nullable, no default — fixes the UTC-vs-athlete-timezone bug class, see session #5 changelog above). **Not yet run** — deliver-for-manual-run.
