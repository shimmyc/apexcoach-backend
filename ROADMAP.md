...
- **Exercise catalog phase 2 — family/muscle-group rollups in Library.** `exercise_catalog.family`/`muscle_groups_primary`/`muscle_groups_secondary`/`equipment` columns exist and are populated (CANONICAL_NAMES seed, MuscleWiki seed once run, and the family heuristic already applied to custom/backfill rows) but are **not yet consumed anywhere** — the queued follow-up is Library rollups (group variants under one family card, e.g. all Push-Up variants) and muscle-group filtering. Blocked on nothing technically; deferred because it's additive UI work, not a fix. See §2/§6.
