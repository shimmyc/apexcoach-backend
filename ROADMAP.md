...
- **🔲 UI (pending)** — backend is complete; none of the following front-end exists yet:
  - **Per-goal roadmap UI** — goal card on the Profile tab expands → inline intake flow (one freeform question at a time) → roadmap view showing `timeline_range` + `date_confidence` caveat, phase cards (near_term: progress bar + `weekly_targets` + `completion_signals`; horizon: `milestone` + `estimated_range`), `adaptation_log`, and a check-in button. Check-in modal: freeform textarea → `POST .../checkin` → roadmap updates in place. Progress bars realistic, capped at 90% until completion signals met (never fake 100%).
  - **Macro roadmap card (Profile tab)** — replaces the current static text roadmap card. Shows overall `timeline_range`, `goals_summary`, phase cards with progress bars, `exercise_gaps` (surfaced as actionable callouts) and `exercise_highlights` (celebrate what's working). Generate/Regenerate button (Sonnet, ~3–5s). Adapts weekly automatically — UI just displays current state.
