...
>
> **⚠ CURRENT CRITICAL ISSUE (2026-06-18 — UNRESOLVED, app non-functional at end of session).** Every Supabase/PostgREST query returns **"Premature close"** (connections dropping mid-request): `daily_checkins`, `profiles`, `wearable_connections`, `workouts`, `roadmap_data`, and raw profile data — **all** affected. The Node server itself is **healthy** (boots, serves requests, and **AI streaming completes fine** — `wroteAny=true`, usage logged correctly); only the Supabase calls fail, so every non-Supabase path appears intact. **Suspected cause:** Supabase connection throttling / soft-pause triggered by the daily-recs **retry storm** (fixed in code this session — see item 5 below), which spiked connection count and/or egress; the Supabase dashboard now shows an **"EXCEEDING USAGE LIMITS"** banner. This is a **paid plan** (734 Micro Compute Hours used, egress looks normal) so it is **not** a free-tier limit — the exact exceeded limit is not yet identified. **Next-session investigation (do these first, before any code change):** (1) check whether the `apexcoach` Supabase project is **paused** — look for a **Resume** button on the project dashboard; (2) Supabase → Settings → Logs → **API logs** — are queries reaching Supabase at all, or being rejected at the edge?; (3) click into the "EXCEEDING USAGE LIMITS" banner to read the **specific** limit; (4) check the **connection-pooler** settings — Render Starter is a single instance, and a pool exhausted by the storm may need a manual reset; (5) if the project is throttled, it may require waiting for the **billing-cycle reset** or contacting **Supabase support**. The retry-storm **root cause is already fixed in code** (item 5) — what remains is recovering the Supabase project state, not a code bug. (Also tracked in §6.)
>
