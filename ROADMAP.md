...
>
> **2026-08-04 session #49 — ✅ THE #48 MATCHER PORTED TO THE ARC + ROADMAP GROUNDING. ONE
> IMPLEMENTATION, THREE EVIDENCE RULES.** Audit-then-build, gated. One commit (`b55c627`),
> `server.js` + one new test file + two existing suites' extractors. **No migration, no client
> change, no new dependency.** Suites **358 → 384**. **Profile 1 `profile_data` sha
> `4313ea6a3896b172b5a2e387627ec6bc1075007f416ba460027c803e8130b9d6` IDENTICAL at session start,
> mid-session, pre-deploy and post-deploy** (93 workouts / 332 exercise rows, both unchanged).
> Profile 4 sha `c356df4b8457d59b`, untouched.
>
> **THE DEFECT.** #48 fixed the substring/tokenization defect for **intake only**, on instruction to
> leave the arc byte-identical. The same defect still governed **(a)** which logged sessions count
> toward a goal's **earned arc position** and **(b)** what grounds roadmap **CONTENT**. Measured on
> profile 1's real 332 rows: a half-marathon goal's arc evidence was **`Crunches` ×3 +
> `Standing Oblique Crunch` ×1 and ZERO runs** (`run` is a substring of "C-**run**-ches"); a bench
> goal was grounded in **`Overhead Press` at 22.5 lb dumbbells**.
>
> **THE PORT.** The matching loop is factored out of `buildTrainingHistorySummary` into shared
> `hxMatchGoalDays()`/`hxExerciseMatch()`. `getGoalSessionDates` is **removed**.
> `extractGoalKeywords` survives for exactly ONE non-evidence consumer, `targetServesGoal`.
> The consumers differ **only in their evidence RULE**: intake reads `matched_dates` (both layers,
> it asks questions); the **arc** reads `specific_dates` ONLY (it drives numbers); **roadmap
> grounding** reads the exercise layer only (`best_set`/`trend` are per-ROW arithmetic).
>
> **⚠ NOT WHOLESALE — the audit is why this session exists.** Handing the arc the raw CATEGORY layer
> would have replaced one false positive with a bigger one: **45** strength sessions for a bench
> goal, **25** cardio sessions for a running goal with zero runs, and — worst — **4 phantom sessions
> each** for **Fix Posture / Fix Pubic Osteitis / Build Muscle**, because `hxAllCategories` falls
> back to `["other"]` for a title naming no category and 4 uncategorised junk workouts
> (`Test Workout`, `Workout` ×2, `Walking`) parse to `["other"]` too.
> **`HX_UNMATCHABLE_CATEGORIES = { other, rest }`** rejects both, everywhere the matcher runs.
>
> **⚠ THE DESCRIPTION FIREHOSE — FOUND IN VERIFICATION, NOT INSPECTION, AND IT REVERSED A DESIGN
> CHOICE MID-BUILD.** The first cut passed `hxGoalTokens(title, description)` into both evidence
> paths, mirroring intake. Measured against real goals that made matching **worse**: a description
> here is freeform coaching prose (profile 1's pinky goal is **~2,000 words**), so tokenising it
> injects ordinary English — the pinky goal reached **`Reverse Lunge`** via `reverse`,
> **`Thoracic Extension Over Foam Roller` via the PREPOSITION `over`**, and `Band Pull-Apart` via
> `band` (4 real matches → 12); **`Stamina`** gained **5** sessions via `full` ("**Full** MMA rounds"
> reaching "Strength (**Full** Body)") and `without` (0 → 5). **Both evidence paths now read the
> goal TITLE only** — which is also what both pre-port paths did, so the port is a pure
> matching-semantics change with an unchanged input contract. Intake is untouched and still reads
> both. **This is a real trade-off, not a free win:** `Fix Posture`'s description genuinely reaches
> `Shoulder Blade Retraction`/`Shoulder Blade Mobility`/`Shoulder Roll` via "rounded shoulders", and
> that signal is given up. Using a description safely needs its own rule — §7 ledger row 54.
>
> **MEASURED ON THE REAL LOG (93 workouts / 332 rows / 75 names):** arc state on all three migrated
> goals **byte-identical** (`position_week 0`, `calendar_week 1`, `drift −1`, `on_track`,
> `qualifying_28d 0`); **7 false positives removed** (bench 3, half-marathon 4); **1 true positive
> gained** (`Pull-Up`); **zero** names added or removed across all 8 real goal titles; the pinky
> goal's 4 matches identical both ways; **0** phantom sessions where there would have been 4 each.
> Rendered intake block **byte-identical on fixtures 10 and 11** (1,087 / 1,063 chars).
>
> **⚠ THE HEADLINE FOR THE ATHLETE: the port changes NOTHING for any goal currently stored, on ANY
> profile — and that is correct.** Profile 4's two arc goals are **tier 1** (linked, so Tier 2 never
> runs); its 3 legacy roadmaps and profile 1's 3 migrated goals match nothing either way; profile 9's
> arc goal has an empty log. **There was consequently NO live behavioural signal available to
> distinguish new code from old** — recorded so a future session does not hunt for one. The value is
> entirely forward-looking: **backdating will replay months of history through a matcher that does
> not count crunches as runs.** An offline simulation (in-memory clones, zero writes) put all three
> goals at `position 0 / calendar 18 / drift −18 / stalled` at an origin of 2026-04-07.
>
> **⚠ AND THE REAL BLOCKER IS NOT THE MATCHER.** All three goals sit at `tier 2` / `keyword` /
> **0 qualifying sessions**, and keyword matching will *never* find "Fix Posture" in an exercise
> name. **Linking each goal to a schedule target is what actually turns evidence on** — and the
> athlete has already looked in the roadmap section and could not find the control. §7 ledger row 50,
> now elevated to the top of the handoff alongside the backdating audit.
>
> **BLAST RADIUS PROVEN:** `public/index.html` is **byte-identical to `aff60c1`** and contains **0**
> references to any ported function, so the daily-rec prompt has a **0-char delta by construction**.
> The dry-run arc sweep is provably write-free (`if (changed && !opts.dryRun)` gates the only PATCH);
> the offline harness recorded and **refused** every non-GET and every Anthropic call — **0 writes,
> 0 AI**.
>
> **TWO HARNESS TRAPS FIRED AGAIN** (both loud, both mine, neither a product bug):
> `assert.deepStrictEqual` **fails across vm realms** (even `[]` vs `[]`), and `stripToCode()`
> **blanks string bodies by design**, so searching its output for a string literal can never match —
> that one fired **twice in one file**. §9.
>
...
> named, and **one of the athlete's own hypotheses was measured and disproved.** The only files
> written this session are `ROADMAP.md` and `CLAUDE.md`. **Profile 1 `profile_data` sha256
> `93971d26be9c89db997120f12299ed0ad8c197211a5484aeb9860550429721ff` at session start AND session
> end**; workouts 92→92, exercise rows 326→326, `daily_recommendations.generated_at` unchanged
> (`2026-08-02T19:57:47.423Z`). Profile 4 untouched. The one browser run used a **network-layer block
> on every non-GET** — 5 POSTs (`evaluate-arcs`, `week-preview`, `goal-progress` ×2, `roadmap-data`)
> were intercepted and never sent — plus stubs for the two read-endpoints that write as a side
> effect (`/daily`, `/micro-goals`).
>
> **D1 — THE APRIL DATA IS NOT GONE. It is a 60-row read cap, and the cap predates every recent
> change.** Counted direct: **92 workouts all-time, 326 exercise rows**, earliest **2026-04-07**.
> Per month: **Apr 24 · May 32 · Jun 13 · Jul 12 · Aug 3** (+8 malformed, below). April carries
> **145 exercise rows across all 24 of its workouts, 42 distinct exercise names, zero null
> `workout_id`** — fully intact. `loadWorkouts()` fetches `/api/workouts?limit=60` ordered `ts.desc`;
> the top 60 reaches back to exactly **2026-05-09**, so **all 24 April workouts plus the 8 oldest May
> workouts — 32 rows — are outside the window on every surface that reads `workoutLog`** (History
> list, calendar, streak, weekly-volume and variety blocks in the rec prompt). The **only** surface
> that can page past it is the Today-tab "Log past workout" panel (`?offset=`, session #21).
> **Nothing in the codebase can delete or overwrite historical workout rows unattended** — every
> delete/overwrite path is user-triggered (see §6).
>
> **⚠ NEW BUG — the malformed-`workouts.date` defect is STILL ACTIVE, not 6 historical rows.**
> §6/§2 record 6 rows holding a TIME STRING in `date` (session #33). There are now **8**, and two
> are recent: ids **100143 (`'10:14'`) and 100144 (`'10:52'`), both `ts` 2026-07-29T19:38Z**, three
> seconds apart. Those rows are invisible to every date-keyed view, are dropped by the arc replay's
> own `Date.parse` filter, and never count toward a streak, a weekly target or a category tally.
>
> **D2 — THE WEEKLY ADAPT IS FIRING. It is the ROADMAP CARD that cannot show it.** All three legacy
> roadmaps carry recent `weekly` entries: **Fix Posture `last_adapted_at 2026-07-30T03:24:00.945Z`,
> v8, 7 log entries**; **Fix Pubic Osteitis `2026-07-27T16:33:35.523Z`, v9, 8 entries**; **Build
> Muscle `2026-07-27T16:34:11.466Z`, v9, 8 entries**. The >7-day gate is working exactly as designed
> (on 2026-07-30 only Fix Posture was stale). **No PT Brain change made the legacy path unreachable**
> — the 2026-07-30 entry postdates sessions #40–#45, and `carryArcForward` is carry-if-present-only.
> **Why it looks frozen, in four parts:** (1) legacy roadmaps use **time-elapsed** progress, so the
> bar moves ~1.8 %/day; (2) phase status is **date-derived** — "Strength & Integration" is pinned
> `current` from 2026-07-09 to 2026-09-02 no matter what is logged; (3) everything the adapt actually
> writes lands in `adaptation_log`, which the card **collapses** behind "Show history (N updates)";
> (4) the adapt is deliberately conservative and has correctly concluded "phase remains current and
> valid" every time. **Also found:** the adapt sees only **`limit=10`** workouts (`date,type`) — its
> whole "weekly review" is computed from ~10 rows. And Build Muscle's 2026-07-27 `weekly` summary is
> **the timeline-estimate basis text, not a weekly review** (§6).
>
> **D3 — THE ATHLETE'S HYPOTHESIS IS WRONG, AND THE REAL CAUSE IS BIGGER. Category is chosen
> UPSTREAM IN CODE, and the strength context is missing for a completely different reason.**
> - **Option 1's activity is not model-chosen.** `buildScheduleInstruction()` picks it: today's
>   anchor, else **the frequency target with the largest raw deficit**, ties broken by array order.
>   Profile 1's targets are **Upper Body Strength 1× · "Lower Body Stength" 1× (typo) · Cardio 2× ·
>   Yoga 2×**, anchors MMA Tue/Thu. A 2×/week target therefore **always outranks a 1×/week target**
>   until it is half-satisfied — and **both strength targets infer to the same category `strength`**,
>   so ONE strength session clears BOTH. **Simulated over the last 14 days against the real log:
>   Yoga was ordered into Option 1 on 7 days, MMA (anchor) on 4, Cardio on 1, and Strength on only
>   the last 2 (2026-08-01 and 2026-08-02).**
> - **It self-corrected today, and the proof is in production.** Today's real cached rec
>   (`2026-08-02T19:57:47Z`) is **all three options strength** — "Push, Pull & Rebuild",
>   "Pull-Dominant Rebuild", "Quick Strength Reset" — because Cardio hit 3/2 ✓ and Yoga 2/2 ✓. The
>   assembled prompt reads verbatim: *"SUGGESTED FOCUS: Upper Body Strength … 0 of 1 done."*
> - **THE TRIM LADDER DOES NOT FIRE AT ALL — measured, both states.** Real `fetchAI({auditOnly:true})`
>   against profile 1: **cold boot 22,779 / 28,000, `_trims: none`, headroom 5,221**;
>   library-warm **27,053 / 28,000, `_trims: none`, headroom 947**. **Nothing is being trimmed away.**
> - **⚠ THE REAL DEFECT: `bootApp()` never calls `loadLibrary()`, so the daily rec is generated with
>   an EMPTY exercise history.** Measured after a full cold boot on Today: **`libExercises = 0`**,
>   `workoutLog = 60`. Consequence in the assembled prompt: **`exerciseHistory` 3,548 → 0 chars**
>   (the entire `RECENT EXERCISE HISTORY` block with every PERSONAL BEST — Dead Hang 2:00, Push-Up
>   3×15, hold PRs — simply absent) and **`recentLog` 765 → 39 chars**, replaced by the literal,
>   **false** line *"No exercises logged in the last 7 days."* — directly contradicting the
>   `THIS WEEK SO FAR` block six lines above it, which says *"Sessions completed: 7."* **4,274 chars
>   of strength and progression signal are silently absent from every cold-boot rec.** This is the
>   same race class §9 flagged as unaudited after session #43's `/goal-progress` fix.
> - **What survives the race, and why today's rec still read well:** the two **briefs** are
>   server-generated from ALL history and are present in both states. "22.5 lbs" comes from
>   `historical_brief`; "last logged strength session was June 22" comes from `coaching_brief`. The
>   briefs are carrying the strength signal by accident — coarsely (one global "22.5 lbs" across
>   rows, curls and presses) and staler (`historical_brief_updated_at 2026-07-15`).
> - **Real last-30-day category mix (code-extracted `inferWorkoutCategory`, 15 workouts):** cardio 7
>   (47 %) · martial_arts 3 (20 %) · mind_body 2 (13 %) · rehab 2 (13 %) · **strength 1 (7 %)**. The
>   recs were reflecting the log — and also causing it.
> - **The manual category pill is WRITE-FREE** — confirmed in source this session: `fetchAI`'s
>   `altOpts` branch sets `altRec`, renders, and **returns before** the `ac_cache` write and before
>   `cacheAIRecOnServer()`. **Not exercised on profile 1** (it still costs a real model call).
>
> **A1 — MIGRATION AND REC FORMAT ARE SEPARABLE, AND THE SANDBOX CONCERN DOES NOT SURVIVE CONTACT
> WITH THE DATA.** The **only** thing a migrated goal injects into the daily-rec prompt is
> `buildArcStateContext()` — one self-capping block, top-3 goals, `ARC_CONTEXT_CHAR_CAP = 950`.
> `capacity` and `coexistence` are read **only** by Profile-tab UI, never by any `fetchAI` builder
> (grep-verified). Rec **format** is governed by `buildResponseShapeSpec` / `buildTimeBudgetContext`
> / `REC_DEPTH_TIERS` / `resolveOptionDurations` — **none of which read goal shape**. **And the
> sandbox comparison was never valid: profile 9 has ZERO workouts, ZERO exercise rows, ZERO
> coaching brief, ZERO historical brief, ZERO micro-goals, and no cached rec at all.** A thin rec
> there is the correct output of a profile with no athlete data — it says nothing about roadmap
> shape. **Recommendation: migrate, but fix the `loadLibrary()` race FIRST.**
>
> **A2 — THE ARC ALREADY READS FULL HISTORY; THE ADAPT AND THE REC DO NOT.** `evaluateArcsForProfile`
> fetches from `arc_origin` forward (`limit=2000` workouts / `8000` exercises) — full history by
> design. **The adapt reads 10 workouts. Goal-progress reads 60 workouts / all-time exercises. The
> daily rec reads 60 workouts + a 7-day exercise slice.** At profile 1's real rate (92 sessions in
> 118 days ≈ **5.5/week**), **60 rows ≈ 11 weeks.** The standing invariant is honored where it
> matters most: the arc position is **computed in code** and the model only narrates it.
>
> **A3 — "HISTORY LIVES FOREVER" IS RECORDED AS A PRODUCT PRINCIPLE.** Nothing deletes history
> unattended; the caps are all **read**-side. Full enumeration, correct-vs-wrong split, and the
> proposal are in §7 and §9.
>
...
> new endpoint, no limit bump.** Profile 1 `profile_data` sha `e28c8c9462aceb81` **identical before
> the session, after the local harness runs, and across the live POST** — the handler is still
> stateless.
>
> **BUG 1 — BOTH STEPS SHIPPED IN ONE SESSION. Body 207,353 B → 86 B (99.96%).**
> - **Step 1** — the client stopped sending `exercises` + `workoutLog`; the handler fetches its own
>   rows. **Step 2** — the client stopped sending `goals`; the handler reads `profile_data.goals`
>   via `loadProfileWithGoals(profileId)`. The per-goal aggregation loop is **untouched**: both
>   steps only repopulate the same three variables it already read.
> - **The windows were replicated, not widened or narrowed** — `workouts` mirrors
>   `GET /api/workouts?limit=60` (`select=*`, `order=ts.desc`, 60) and `exercises` mirrors
>   `GET /api/profiles/:id/exercises` (`select=*`, `order=date.desc`, `limit=5000` = all-time).
>   **The client's `workoutLog.slice(0, 90)` never actually bound anything** — `loadWorkouts()`
>   fetches at `limit=60`, so 60 was always the real ceiling. The client's `exercises` array was
>   that endpoint's grouped `sessions[]` flattened, and those are the raw rows pushed verbatim
>   (`server.js:3933`), so the row shape is identical.
...
>   `node-fetch` stubbed via `require.cache`, and the actual express route was exercised over HTTP
>   against the same real read-only profile-1 fixture (8 goals, 60 workouts, 310 exercise rows). No
>   source slicing, so there is no extraction bug class. Results:
>   **response JSON byte-identical for all 8 goals** (sha `f5077b6076893503`, only the
>   deliberately-varying `last_computed_at` normalised); **all 5 AI prompts byte-identical**
>   (sha `287284ee51079508` — the two AI branches are non-deterministic at real temperature, so
>   identity was proven on the assembled prompts, which is where every aggregate the arrays feed
>   actually lands); **zero Supabase writes on both paths.**
> - **The pre-fix server REPRODUCED BUG 1 LOCALLY, verbatim** — the first harness run returned
>   `PayloadTooLargeError` and the client-side `SyntaxError: Unexpected token '<', "<!DOCTYPE "`.
>   The body limit was then raised **identically in both harness copies** purely to obtain a
>   baseline; the diff between the two copies is exactly the fix.
> - **Two extra hardening checks, both pass:** a stale cached client still POSTing the 202.5 KB body
>   gets **byte-identical output** (the server ignores the posted arrays); a simulated Supabase
>   outage degrades to **200 with `progress: []`**, never a 500.
> - **VERIFIED LIVE post-deploy** (production, profile 1, 86-byte body): HTTP **200**, 8 goals
>   returned. The three deterministic goals are **byte-identical to the pre-fix baseline** —
>   `skill` `54% "blue1 → Black Belt (7/13)"`, `habit` `0% "0/30 days this month"`, `consistency`
>   `56% "9 sessions this month / 16 target"`. The five AI goals return the same branch/`source`
>   with live Haiku percentages (non-deterministic by design). `profile_data` sha unchanged across
>   the live POST — **the handler still writes nothing.**
> - **⚠ NEW FINDING, and it makes the fix strictly more correct than "identical".** `fetchGoalProgress()`
>   is reached from `renderProfileGoals()`, which `bootApp()` calls via `renderProfileTab()` — and
>   `bootApp()` **never calls `loadLibrary()` at all**, while `loadWorkouts()` is still in flight.
>   So on a cold boot the pre-fix endpoint was frequently sent **empty `exercises` and empty
>   `workoutLog`**, silently computing strength/distance/consistency goals against no data. The
>   pre-fix behaviour was therefore **non-deterministic, dependent on whether the athlete had
>   visited the Library tab.** Server-side fetching removes the race entirely. Logged in §9.
>
> **LEDGER ROW 22 — CLOSED, VERIFIED LIVE. The approved nudge was NOT NEEDED.** Profile 4's arc
> `last_evaluated` was already **61.35 h** stale (`2026-07-25T22:57:33.897Z`), i.e. the ledger's own
> primary route ("let >24h pass") was already satisfied — so the stale branch was exercised with
> **zero setup writes** to profile 4. `POST /api/profiles/4/evaluate-arcs` returned
> `{skipped:false, evaluated:2}` in **1.17 s**; an immediate re-fire returned
> `{skipped:true, reason:"fresh", evaluated:0}` in 0.44 s, proving the run did not leave the goal
> stuck stale. Wrist-goal `arc_state` before → after: `last_evaluated 2026-07-25T22:57:33.897Z →
> 2026-07-28T12:18:32.807Z`, `calendar_week 4 → 5`, `drift −4 → −5`, `flex_streak 14 → 15`,
> evidence `tier 2/keyword/matched_via [] → tier 1/category/["target:Rehab right wrist
> tendonitis"]`, `qualifying_sessions_28d 1 → 2`, `longest_gap_days 22 → 20`;
> `position_week 0`, `status "stalled"`, `re_ramp null`, `needs_regeneration true` and
> **`arc_origin 2026-06-29` all unchanged**. **This closes ROW 22 ONLY — it does not touch row 28.**
> - **ZERO AI CALLS — MEASURED, NOT ASSUMED.** The real shipped server was booted with `node-fetch`
>   stubbed and the real `/evaluate-arcs` route fired against profile 4's real pre-evaluation
>   snapshot: the route made exactly **5 outbound requests — 4 Supabase GETs and 1 Supabase PATCH,
>   and ZERO requests to `api.anthropic.com`.** Corroborated live: `roadmap.version` 2→2,
>   `adaptation_log` 1→1, `last_adapted_at` unchanged, `phases` unchanged — an adapt is the only AI
>   path reachable from here and it demonstrably did not run.
> - **The recompute is a PURE REPLAY — proven, not asserted.** That same offline replay reproduced
>   the arc_state production wrote **exactly**, for both goals, every field except the wall-clock
>   `last_evaluated`. (Compared key-order-insensitively: a jsonb round-trip reorders object keys, so
>   a naive `JSON.stringify` diff reports a false mismatch — noted so it is not re-debugged.)
>
> **ROW 28 / item (c) — re-labeled per decision, NOT closed, closing check UNCHANGED.** No date is
> asserted for the bench goal's destroyed origin; it stays unrecoverable and was not guessed
> (§0.2 rule 5). **New evidence this session, unplanned:** the bench goal **re-acquired an
> `arc_state` during the row-22 evaluation** — `position 0, calendar_week 2, drift −2, confidence
> precise, tier 1, needs_regeneration false` — with its origin now pinned at the **2026-07-26
> regeneration date**. That is the BUG 3 residual materialising exactly as predicted, and it means
> **route (1) is now live and protected**: from 2026-07-26 forward the session-#42 fix guarantees
> the origin no longer walks, so real qualifying sessions can accumulate toward a genuine
> peak-then-gap. Nothing was synthesised.
>
> **STANDING RULE RECORDED: no body-limit bump on `/goal-progress`, ever, in any form.** It is a
> stopgap against a payload that grows with every logged row; the endpoint is now <1 KB and
> migration-proof, so there is no scenario in which raising the limit is the right answer.
>
...
### Session #49 findings (2026-08-04)

- **✅ RESOLVED — the arc's Tier-2 matcher and `getGoalExerciseContext` now run the #48 matcher.**
  §6's session-#48 entry recorded this as "measurably broken, quantified, deliberately NOT fixed".
  It is fixed: `getGoalSessionDates` is gone, both paths call the shared
  `hxMatchGoalDays()`/`hxExerciseMatch()`, and the before/after is measured row by row on profile 1's
  real log. `extractGoalKeywords` remains only for `targetServesGoal`.
- **⚠ A GOAL DESCRIPTION MUST NOT BE TOKENISED INTO EVIDENCE — found by measurement, and it reversed
  a design choice mid-build.** Descriptions in this app are freeform coaching prose of arbitrary
  length (profile 1's pinky goal carries **~2,000 words**), so every ordinary English word in them
  becomes a matching token. Measured: **`Thoracic Extension Over Foam Roller` matched a HAND-rehab
  goal through the preposition `over`**; `Reverse Lunge` through `reverse`; `Band Pull-Apart` through
  `band`; and **`Stamina` gained 5 sessions through `full`**, because "Full MMA rounds" and
  "Strength (**Full** Body)" share a word. Both evidence paths read the **title only**. **Intake
  still reads both and is unchanged** — a loose match there costs a slightly worse question, not a
  wrong number. **This is a genuine trade-off, not a free win:** `Fix Posture`'s description reaches
  `Shoulder Blade Retraction` / `Shoulder Blade Mobility` / `Shoulder Roll` through "rounded
  shoulders", and that real signal is given up. A safe description rule (e.g. admit only description
  tokens that appear in the exercise-name vocabulary, or cap them) is scoped, not built — row 54.
- **⚠ `hxAllCategories` NEVER returns empty — it falls back to `["other"]`, and `other` is not a
  category.** A goal title naming no category ("Fix Posture", "Build Muscle") therefore matched every
  uncategorised workout: measured, **4 phantom sessions each** on profile 1 (`Test Workout`,
  `Workout` ×2, `Walking`). Fixed by `HX_UNMATCHABLE_CATEGORIES = { other, rest }`. **This changed
  INTAKE too, and for the better** — 6 of profile 1's 8 goals now show an honest *"NO logged sessions
  match"* where they previously claimed 2–4 category-level sessions that were Test Workout /
  Meditation / Walking. Fixtures 10/11 (both `cardio` goals) are byte-identical.
- **⚠ THE PORT IS A NO-OP ON EVERY GOAL CURRENTLY STORED, ON EVERY PROFILE — so there was no live
  behavioural signal to distinguish new code from old.** Profile 4's two arc goals are **tier 1**
  (linked, so Tier 2 never runs) and its three legacy roadmaps match nothing either way; profile 1's
  three migrated goals match nothing either way; profile 9's arc goal has an empty log; fixtures
  10/11 have no roadmaps and so no arc. Post-deploy verification is therefore a confirmation that the
  endpoints return the expected values, **not** independent proof that the new build is serving —
  the behavioural proof is the offline before/after against the real shipped code from both file
  versions. **Recorded so a future session does not spend time hunting for a probe that cannot
  exist.**
- **The arc's evidence window is bounded by `arc_origin`, which is why the port had to precede
  backdating.** All three of profile 1's migrated goals have `arc_origin 2026-08-03`, so
  `evaluateArcsForProfile` fetches from that date forward — **~1 day, 1 logged session**. Even a
  perfect matcher sees almost nothing today. Backdating without this fix would have replayed months
  of history through the crunch-matching matcher and produced a confidently wrong earned position
  instead of an honestly zero one.
- **`duration_minutes` overload — FOURTH sighting, now accruing as tracked tech debt.** The column
  means BOTH hold duration AND session length, with nothing in the schema distinguishing them.
  Engine v2 worked around it by category; session #48 worked around it by category for a display
  label; this session inherits that same workaround inside `renderTrainingHistoryBlock`. **Three
  separate features have now had to implement the same category heuristic.** Scoped proposal in §9 —
  **not built.**
- **Harness trap, re-confirmed: `stripToCode()` blanks string bodies BY DESIGN.** Searching its
  output for a string literal (`confidence: "keyword"`, `hxGoalTokens(title, "")`) can therefore
  never match, and the assertion fails looking like a product regression. **Fired twice in one
  file.** Use the RAW slice for literal checks and the stripped form for identifier/call checks.
- **Harness trap, re-confirmed: `assert.deepStrictEqual` fails across vm realms** — even `[]` vs `[]`
  throws "same structure but not reference-equal", because the sandbox array's prototype is not the
  test realm's. Already recorded in §9 from session #45; it caught a fresh victim here. Compare a
  serialised form.
...
> with zero writes to profile 1 (`profile_data` sha256 identical at both ends of the session).

- **⚠ THE DAILY REC IS GENERATED WITH AN EMPTY EXERCISE HISTORY ON EVERY COLD BOOT — the single
  highest-impact finding of this session, and it is NOT the trim ladder.** `bootApp()` never calls
  `loadLibrary()` (call sites are `showTab('library')` and the Library-tab renderers only), and
  `libExercises` is the sole source for **both** `buildLog()` (the `RECENT N-DAY LOG`) and
  `buildExerciseHistory()` (the `RECENT EXERCISE HISTORY` block that carries every PERSONAL BEST).
  **Measured in a real browser after a full cold boot on Today: `libExercises = 0`, `workoutLog = 60`.**
  Assembled-prompt delta, real `fetchAI({auditOnly:true})` on profile 1:

  | section | cold boot | after `loadLibrary()` |
  |---|---|---|
  | `exerciseHistory` | **0** | 3,548 |
  | `recentLog` | **39** | 765 |
  | `_total` | 22,779 / 28,000 | 27,053 / 28,000 |
  | `_trims` | **none** | **none** |

  The 39-char cold value is the literal string **"No exercises logged in the last 7 days."** — which
  is **false**, and sits six lines below a `THIS WEEK SO FAR` block stating *"Sessions completed: 7."*
  So the prompt actively contradicts itself. **4,274 chars of strength/progression signal are absent
  from the rec the athlete actually receives**, including Dead Hang 2:00, Push-Up 3×15, and every
  hold PR. **This is the same race class §9 flagged as unaudited after session #43** fixed it for
  `/goal-progress` — that fix moved the reads server-side for that one endpoint and explicitly noted
  the other `libExercises`/`workoutLog` read sites had never been checked. This is one of them.
  **Partial mitigation, discovered while measuring:** the `coaching_brief` (2,365 chars) and
  `historical_brief` (2,157) are **server**-generated from all-time history and survive the race —
  they are why today's rec could still cite "22.5 lbs" (from `historical_brief`) and "last logged
  strength session was June 22" (from `coaching_brief`). The briefs are carrying the progression
  signal by accident, coarsely and staler (`historical_brief_updated_at 2026-07-15`).
- **✅ MEASURED AND DISPROVED — the daily-rec prompt is NOT being trimmed.** The hypothesis that
  trimming was dropping strength context was tested with a real `auditOnly` run, not reasoned about:
  **`_trims: none` in both the cold and the warm state.** Headroom 5,221 cold / 947 warm. **⚠ Two
  sections were deliberately suppressed in that run to guarantee zero writes** — `/api/profiles/1/daily`
  (which fire-and-forget upserts `daily_steps`/`daily_sleep` and stamps `last_synced_at`) and
  `/api/profiles/1/micro-goals` (which PATCHes `current_value` when it changes). So `microGoals`
  read 0 where session #39 measured **1,347**, and `biometrics` rendered with `?` placeholders at
  410. **Adding session #39's `microGoals` figure back puts the warm untrimmed total at ~28,400,
  i.e. ~400 over budget — enough to fire exactly ONE rung, `historicalBrief->400`, which matches
  what #39 reported.** Even then, `exerciseHistory` is rung 3 and is **never reached**. Conclusion
  stands: trimming removes nothing that carries strength context.
- **⚠ THE MALFORMED-`workouts.date` DEFECT IS STILL ACTIVE — there are now 8 rows, not 6, and two
  are five days old.** §2 and the session-#33 clone notes record 6 profile-1 rows holding a TIME
  STRING in the `text` `date` column. The real count today is **8**:

  | id | stored `date` | real `ts` (UTC) | type |
  |---|---|---|---|
  | 77 | `10:20` | 2026-05-28 16:53 | Martial Arts |
  | 82 | `10:07` | 2026-06-02 16:17 | Martial Arts |
  | 88 | `19:37` | 2026-06-10 03:32 | Yoga |
  | 95 | `13:00` | 2026-06-18 13:00 | Walk |
  | 97 | `12:52` | 2026-07-15 13:03 | Workout |
  | 110 | `10:12` | 2026-07-21 19:24 | Martial arts |
  | **100143** | `10:14` | **2026-07-29 19:38** | Martial arts |
  | **100144** | `10:52` | **2026-07-29 19:38** | Walk |

  The last two were written **three seconds apart** and are the first evidence the defect is
  **recurring, not historical**. Consequences, all silent: the rows never render on the calendar or
  in any date-keyed view; `evaluateArcsForProfile` drops them at its own `Date.parse` filter; they
  never count toward a streak, a weekly target, or a category tally; and `dedupe-workouts` keys on
  `date + '|' + ts`, so a future pair sharing both would be deleted as a duplicate. **The write path
  that produces them was NOT traced this session** — that is the first step when this is picked up.
  (`POST /api/workouts` forwards `req.body` verbatim and its future-date guard only fires when
  `body.date` parses, so a time string passes through.)
- **The weekly roadmap adapt IS running on profile 1; the roadmap CARD structurally cannot show it.**
  Evidence: `last_adapted_at` **2026-07-30T03:24:00.945Z** (Fix Posture, v8), **2026-07-27T16:33:35.523Z**
  (Fix Pubic Osteitis, v9), **2026-07-27T16:34:11.466Z** (Build Muscle, v9), each with a `weekly`
  `adaptation_log` entry containing genuinely new analysis. Four independent reasons the athlete sees
  nothing: (1) legacy roadmaps take the **time-elapsed** progress branch (`arc_state` absent), capped
  at 90 % — ~1.8 %/day on an 8-week phase; (2) phase status is **date-derived**, so "Strength &
  Integration" is `current` from 2026-07-09 to 2026-09-02 regardless of what is logged; (3) the new
  analysis lands **only** in `adaptation_log`, which `renderGoalRoadmap` collapses behind
  "Show history (N updates)"; (4) the adapt prompt's conservative bias is working — it has correctly
  answered "phase remains current and valid" every time. **Not a bug. A visibility gap.**
- **`maybeAdaptAllRoadmaps` judges a "weekly review" from 10 workout rows.** `server.js:7534` fetches
  `order=date.desc&limit=10&select=date,type`. At profile 1's real rate (~5.5 sessions/week) that is
  about **12 days**, and `buildWeeklyReviewContext`'s own `prev14` bucket (days 15–28) is therefore
  almost always empty by construction — the function guards its `prev14 === 0` resumption signal
  against exactly this, but the 14-day bucket itself is still built from a window that barely reaches
  14 days. Per-goal exercise context is separately fetched at 90 days and full context at 60, so the
  adapt is not blind — but its *session-count* evidence is thinner than the prose it writes implies.
- **Build Muscle's 2026-07-27 `weekly` adaptation_log summary is not a weekly review.** It reads
  *"Visible muscle definition and functional size typically require 6-12 months of consistent
  progressive overload…"* — that is timeline-estimate **basis** prose, not an analysis of the week.
  The other two goals' `weekly` entries that day are real reviews. Single occurrence, not chased.
- **Nothing in the codebase can delete or overwrite historical workout/exercise rows unattended —
  enumerated in full.** Every path is user-triggered or admin-gated:

  | path | effect | trigger | guard |
  |---|---|---|---|
  | `DELETE /api/workouts/:id` | deletes 1 workout + its exercises | History card "Delete" | client `confirm()` |
  | `DELETE /api/profiles/:id` | deletes ALL workouts + exercises | Settings → delete profile | PIN in body |
  | `POST /api/profiles/:id/dedupe-workouts` | **bulk DELETE**, keyed `date\|ts` | Profile dev button | **⚠ NO confirm, NOT admin-gated** |
  | `POST /api/profiles/:id/reformat-titles` | **overwrites `workouts.type` on every workout with notes** (limit 10000, one AI call each) | Profile dev button | client `confirm()` |
  | `PATCH /api/workouts/:id` | edits one row (never re-extracts — §6 above) | History card edit | — |
  | `DELETE /api/profiles/:id/exercises/:exerciseId` | 1 exercise row | admin/audit tool | profile-ownership check |
  | `POST /api/debug/delete-orphaned-exercises/:id` | reviewed orphan list | admin | `ADMIN_SECRET` + re-verify |
  | `POST /api/debug/purge-sandbox-workouts/:id` | `[SEED]`-marked rows only | admin | `ADMIN_SECRET` + **profiles 1 and 4 denylisted** |

  **No scheduler, cron, or fire-and-forget path deletes or rewrites a workout or exercise row.** The
  v2 nightly job deletes only `planned_sessions`. **Two items worth flagging (both logged in §9):**
  `dedupe-workouts` performs a bulk delete from a single unconfirmed tap, and `reformat-titles`
  rewrites the `type` of every historical workout — and `type` is exactly what
  `inferWorkoutCategory()` reads for every weekly-target count and category tally.
- **Option 1's CATEGORY is chosen upstream in code, and the scorer has no notion of goal priority.**
  `buildScheduleInstruction()` picks today's anchor, else the `frequency_target` with the largest
  raw deficit (`needed - done`), `+0.5` only if today is its `suggested_day`, ties broken by array
  order via a strict `>`. Profile 1's four targets are **Upper Body Strength 1× · "Lower Body
  Stength" 1× · Cardio 2× · Yoga 2×**, all with `suggested_day: null`. Three compounding effects:
  (a) a **2×/week target structurally outranks a 1×/week target** until it is half-met, so Yoga and
  Cardio monopolise Option 1; (b) **both strength targets resolve to the same inferred category
  `strength`**, so `schedTargetDoneCount` counts the same workouts for both and ONE strength session
  clears BOTH; (c) `inferWorkoutCategory` tests `strength` **before** `cardio`, so a combined title
  like *"Cardio (Stationary Bike, 20min) + Strength (Upper Body) + Mind & Body"* counts as **strength
  only** and advances neither the cardio nor the yoga target. **Simulated over the real last 14 days:
  Yoga ordered 7×, MMA anchor 4×, Cardio 1×, Strength 2× (2026-08-01 and 2026-08-02 only).** Not a
  bug in any single function — an emergent property of `times_per_week` × a raw-deficit scorer. The
  athlete can change the outcome today by editing the targets; no code change is required for that.
- **`frequency_targets[1].activity` is spelled `"Lower Body Stength"` in profile 1's stored
  schedule.** It still resolves to `strength` (the regex matches `lower`), so nothing breaks — but it
  renders verbatim in the prompt's `WEEKLY TARGET STATUS` block and in the Schedule card. **Athlete
  data, not code. Not corrected this session (read-only scope).**
- **The sandbox comparison that motivated the "roadmaps only" migration request was never valid.**
  Profile 9 has **0 workouts, 0 exercise rows, 0 coaching brief, 0 historical brief, 0 micro-goals,
  and no `daily_recommendations` row at all** — so no artifact of the rec the athlete judged even
  exists to re-read. Its one roadmap'd goal (Bench Press 185lb, `strength_load`, `arc_origin
  2026-07-28`) sits at arc week 0. A generic rec on that profile is the correct output of a profile
  with no athlete data and carries no information about whether the migrated roadmap shape degrades
  the rec.
...
**BUG 1 — HTTP 413 on `GET`/`POST /api/profiles/1/goal-progress` (profile 1, live). ✅ FIXED AND
VERIFIED LIVE 2026-07-28 (session #43), commit `898fd02` — see "BUG 1 — RESOLVED" below. AUDITED
2026-07-27 (session #42); the audit numbers stay recorded as the basis for the fix and are NOT
superseded. Original entry kept as the record of the investigation.**
...
#### ✅ BUG 1 — RESOLVED (2026-07-28, session #43), commit `898fd02`. Both steps shipped, verified live.

> The AUDIT RESULT block that follows is **still authoritative as the basis** for this fix — every
> number in it was re-measured independently this session and matched (207,353 B vs the audit's
> 207,357 B; the 4-byte delta is the two scalar values, not a structural difference).

**What shipped** — `server.js` + `public/index.html` only. No migration, no schema change, no new
endpoint, **no limit bump**.

| step | change | body |
|---|---|---|
| baseline | client posts `goals` + `workoutLog` + `exercises` | **207,353 B (2.02×)** — 413s |
| **step 1** | client stops sending `exercises` + `workoutLog`; handler fetches its own rows | 37.6 KB |
| **step 2** | client stops sending `goals`; handler reads `profile_data.goals` via `loadProfileWithGoals()` | **86 B (99.96% reduction)** |

- **The per-goal aggregation loop was not touched.** Both steps only repopulate the same three
  variables (`goals`, `workouts`, `exercises`) the loop already read. Three small helpers were
  added above the route — `fetchGoalProgressWorkouts`, `fetchGoalProgressExercises`,
  `loadGoalProgressGoals` — and the three `body.*` reads became one `Promise.all`.
- **Windows REPLICATED, not widened or narrowed.** `workouts` mirrors `GET /api/workouts?limit=60`
  (`select=*`, `order=ts.desc`, `limit=60`); `exercises` mirrors
  `GET /api/profiles/:id/exercises` (`select=*`, `order=date.desc`, `limit=5000` = all-time).
  **The client's `workoutLog.slice(0, 90)` never bound anything** — `loadWorkouts()` fetches at
  `limit=60`, so 60 was always the real ceiling. The client's `exercises` array was that endpoint's
  grouped `sessions[]` flattened, and `sessions` holds the raw rows pushed verbatim
  (`server.js:3933`), so the row shape is identical.
- **Non-fatal by design.** Each read degrades to `[]` on failure — the same state the client could
  already deliver — so a Supabase blip returns `200` with `progress: []`, never a 500. Proven.
- **Back-compat.** `loadGoalProgressGoals` falls back to `body.goals` only if the profile read
  fails. Arrays still posted by a stale cached client are **deliberately ignored**; the server copy
  is authoritative and deterministic.
- **Still stateless.** Zero writes, verified three ways: no write exists in the handler; the
  harness recorded zero non-GET Supabase requests on both the old and new paths; and profile 1's
  `profile_data` sha `e28c8c9462aceb81` is unchanged across the live POST.

**Verification, before deploy.** The REAL pre-fix server (`git show HEAD:server.js`) and the REAL
post-fix server were each booted in-process with `node-fetch` stubbed via `require.cache`, and the
actual express route exercised over HTTP against the same real read-only profile-1 fixture (8
goals, 60 workouts, 310 exercise rows). **No source slicing**, so the arc close-out learning #2
extraction bug class does not apply. Anthropic was stubbed with a fixed reply because the two AI
branches are non-deterministic at real temperature — identity was therefore proven on the
**assembled prompts**, which is precisely where every aggregate the arrays feed lands.

| check | result |
|---|---|
| pre-fix server reproduces the bug | **`PayloadTooLargeError` → 413 → `SyntaxError: Unexpected token '<', "<!DOCTYPE "`, verbatim** |
| response JSON, all 8 goals, old vs new | **byte-identical**, sha `f5077b6076893503` (`last_computed_at` normalised — it is `new Date()`) |
| all 5 AI prompts, old vs new | **byte-identical**, sha `287284ee51079508` |
| Supabase writes, both paths | **0 / 0** |
| stale client still POSTing 207,353 B | byte-identical output; arrays ignored |
| simulated Supabase outage | `200` + `progress: []`, no 500 |
| existing suites | **213/213** |

To obtain the old-path baseline at all, the body limit was raised **identically in both harness
copies** (`express.json({limit:"50mb"})`); the diff between the two copies is then exactly the fix.

**Verification, live post-deploy** (production, profile 1, 86-byte body): HTTP **200**, 8 goals.
The three deterministic goals are byte-identical to the pre-fix baseline — `skill` `54%
"blue1 → Black Belt (7/13)"`, `habit` `0% "0/30 days this month"`, `consistency` `56% "9 sessions
this month / 16 target"`. The five AI goals hit the same branch and `source` with live Haiku
percentages (non-deterministic by design). `profile_data` sha unchanged across the POST.
...
`renderProfileTab()` — but **`bootApp()` never calls `loadLibrary()`**, and `loadWorkouts()` is
still in flight at that point. So on a cold boot the pre-fix endpoint was frequently sent **empty
`exercises` and empty `workoutLog`**, computing strength / distance / consistency goals against no
data at all. Pre-fix behaviour was **non-deterministic, contingent on whether the athlete had
visited the Library tab in that session**. Server-side fetching removes the race. Logged in §9.

**STANDING RULE: never raise the body limit for this endpoint, in any form.** It is a stopgap
against a payload that grows with every logged exercise row forever. The body is now 86 bytes and
immune to both history growth and goal-shape growth, so no scenario makes a bump correct.

#### BUG 1 — AUDIT RESULT (2026-07-27, session #42). Measured, read-only, zero writes. ✅ FIX SHIPPED 2026-07-28 (session #43) — retained as the basis.
...
| 22 | L3 | App-open arc evaluation — **STALE** branch | **✅ CLOSED — VERIFIED LIVE (2026-07-28, session #43)** | **The approved back-dating nudge was NOT needed** — profile 4 was already **61.35 h** stale (`last_evaluated 2026-07-25T22:57:33.897Z`), satisfying the check's own primary route, so the branch was exercised with **zero setup writes**. `POST /api/profiles/4/evaluate-arcs` → `{skipped:false, evaluated:2}` in **1.17 s**; immediate re-fire → `{skipped:true, reason:"fresh", evaluated:0}` in 0.44 s, so the run did not leave the goal stuck stale. Wrist goal `arc_state` before → after: `last_evaluated 2026-07-25T22:57:33.897Z → 2026-07-28T12:18:32.807Z`, `calendar_week 4 → 5`, `drift −4 → −5`, `flex_streak 14 → 15`, evidence `tier 2/keyword/[] → tier 1/category/["target:Rehab right wrist tendonitis"]`, `qualifying_sessions_28d 1 → 2`, `longest_gap_days 22 → 20`; `position_week 0`, `status "stalled"`, `re_ramp null`, `needs_regeneration true`, **`arc_origin 2026-06-29`** all unchanged. **ZERO AI CALLS — MEASURED:** the real shipped server was booted with `node-fetch` stubbed and the real route fired against profile 4's real pre-evaluation snapshot — exactly **5 outbound requests, 4 Supabase GETs + 1 Supabase PATCH, ZERO to `api.anthropic.com`**; corroborated live by `roadmap.version` 2→2, `adaptation_log` 1→1, `last_adapted_at` and `phases` unchanged (an adapt is the only reachable AI path). **PURE REPLAY PROVEN:** that offline replay reproduced production's arc_state **exactly** for both goals, every field except the wall-clock `last_evaluated`. **This closes ROW 22 ONLY — it does not touch row 28.** |
...
| 28 | **L4 — item (c)** | **A re-ramping goal produces a visibly lighter, rebuild-the-base rec** | **⚠ OPEN — WAITING ON REAL ACCUMULATED DATA** (re-labeled 2026-07-28, session #43; route (1) chosen by decision). Criterion (iii) PASSES; (i) and (ii) remain open. **Not blocked on code, and not blocked on any other row — there is nothing to build and nothing to nudge** | **No longer blocked by BUG 2** (resolved — designed check-in gate) and the generation path is proven: two REAL model calls were made on profile 4 through the **category path**, which `fetchAI` short-circuits into `altRec` and returns from **before** any `localStorage` write and before `cacheAIRecOnServer` — **provably write-free**, confirmed at runtime (zero non-GET calls besides `/api/ai`). **A** = arc block present (628 chars, prompt 27,120); **B** = identical state with the arc fields stripped in memory only (0 chars, 26,492); 628-char delta. **(iii) PASSES — but vacuously, and that is stated, not glossed:** the rec contains **no position / drift / week number at all**, so it cannot state one that was not injected; it does not prove the model would resist inventing a number while narrating progress. **(i)+(ii) cannot be judged from this pair** — the only arc goal on profile 4 is `stalled`, **not** `re_ramping`, so the block's re-ramp instruction never applied. Structurally the arc run came back marginally **denser**, not lighter (opt1 16 movements/~33 sets vs 14/~28), the three "rebuilding" hits are about **pubic osteitis** not the arc goal, and **the arc goal never appears in `goal_tags` in EITHER run**. **REMAINING CHECK, and it needs real data:** a goal must actually reach `re_ramping`, which `computeArcState` grants only when `preGapPeak > decayed` — i.e. a real earned peak followed by a gap. The wrist goal's peak is 0, so the back-dated-`last_evaluated` nudge **cannot** produce one (the replay is pure and deterministic; see row 22, which that nudge WOULD close). The bench goal could, but BUG 3 destroyed its `arc_origin` and it is **not recoverable** (see row 31). **Two honest routes: (1) let profile 4 accumulate real qualifying sessions on the bench goal until a genuine peak-then-gap forms — the BUG 3 fix now protects the origin; (2) the athlete states the bench roadmap's intended phase-1 start date as a product decision, after which the replay derives everything else from the real log.** **✅ DECISION RECORDED 2026-07-28 (session #43): ROUTE (1).** No date will be asserted for this goal's origin — it is unrecoverable and will NOT be guessed (§0.2 rule 5). Profile 4 accumulates real qualifying sessions until a genuine peak-then-gap forms. **The exact closing check above is UNCHANGED**; what changed is that there is now nothing to build and nothing to nudge — only real data to wait for. **New evidence, unplanned (session #43):** the row-22 arc evaluation on 2026-07-28 caused the bench goal to **re-acquire an `arc_state`** — `position_week 0, calendar_week 2, drift −2, confidence precise, tier 1, re_ramp null, needs_regeneration false` — with its origin now pinned at the **2026-07-26 regeneration date**. That is the BUG 3 residual materialising exactly as predicted, and it means **route (1) is live and protected from 2026-07-26 forward**: the session-#42 fix guarantees the origin no longer walks on adapt/regenerate, so earned weeks can now actually accumulate. **Do NOT synthesise a re-ramp.** |
...
| 42 | **Rec prompt** | **The trim ladder is NOT removing strength context — the athlete's hypothesis, measured and DISPROVED** | **✅ VERIFIED (2026-08-02, session #46)** | Real `fetchAI({auditOnly:true})` on production profile 1 from a browser under a network-layer non-GET block: **cold boot 22,779 / 28,000 `_trims: none` (headroom 5,221)**; library-warm **27,053 / 28,000 `_trims: none` (headroom 947)**. `timeBudget` 2,180 cross-checks exactly against #39's 1,539 + Layer 4's +641. **Caveat stated, not glossed:** `/daily` and `/micro-goals` were stubbed because both WRITE as a side effect, so `microGoals` read 0 (vs #39's 1,347) and `biometrics` rendered with `?` placeholders at 410 — adding #39's `microGoals` figure back puts the warm untrimmed total ~400 over budget, enough for exactly ONE rung (`historicalBrief->400`), matching #39. **`exerciseHistory` is rung 3 and is never reached either way** |
| 43 | **Rec prompt — NEW DEFECT** | **`bootApp()` never calls `loadLibrary()`, so the daily rec is generated with an EMPTY exercise history** | **⚠ OPEN — MEASURED, NOT FIXED (2026-08-02, session #46)** | Real browser, cold boot on Today, 9 s settle: **`libExercises = 0`**, `workoutLog = 60`. Assembled prompt: **`exerciseHistory` 3,548 → 0 chars** (the whole `RECENT EXERCISE HISTORY` block with every PERSONAL BEST) and **`recentLog` 765 → 39 chars**, the 39 being the literal **false** line *"No exercises logged in the last 7 days."* six lines below *"Sessions completed: 7."* **4,274 chars of strength/progression signal absent from every cold-boot rec.** Second instance of the race session #43 fixed for `/goal-progress`; §9 predicted more existed. **Partial backstop found:** the two briefs are server-generated from all-time history and survive, which is why a cold rec can still cite "22.5 lbs" (`historical_brief`) and "June 22" (`coaching_brief`). **CHECK TO CLOSE:** after the fix, a cold-boot `auditOnly` run must report `exerciseHistory > 0` and a `recentLog` that lists real sessions. §9 |
...
| 50 | **L2 — migration follow-up** | **Migrated goals are at arc `tier 2` / `keyword` confidence with 0 qualifying sessions** | **⚠ OPEN — NEEDS THE ATHLETE. ⚠ THE #49 CLOSE-OUT DESCRIBED THE UI PATH INCOMPLETELY; CORRECTED 2026-08-04 after a source trace + a real read-only browser probe.** | None of profile 1's goals is linked to a schedule item, so the keystone join (`frequency_targets[].goal_ids`) is unused and the arc matches by keyword alone. **The control DOES exist, IS deployed and DOES render** — verified in a headless browser against production with every non-GET blocked: **6 pickers, 48 pills, visible**, at both 1440×1200 and 390×844. `schedGoalPickerHtml` (`public/index.html:12499`) is called from the weekly-target row (`:12491`), the addon row (`:12601`) and the open-anchor editor (`:12412`); the write is `schedToggleGoalLink` (`:12526`) → `schedPersist()`. **Why the athlete saw nothing: it is the LAST element inside each Weekly Target row** — below the activity input, 🗑, the stackable checkbox, its italic explainer, the x/week stepper, the day dropdown and the duration dropdown — and at 390 px the first one sits **448 px below the top of the Schedule card**, i.e. just past the first fold (a screenshot shows the viewport cutting off at the stackable checkbox). Its label is **"Serves which goals?"** — the words *goal selector*, *link* and *target* never appear as a heading. **Its ONLY precondition is `currentProfileData.goals.length > 0`, and each goal needs a `g.id`** (`:12500`, `:12506`); it is NOT gated on roadmap, arc_state, new-shape, `goal_type` or tab state. **Check:** Profile → Schedule → "YOUR TRAINING BLUEPRINT ▸ Edit" → scroll **inside** the first Weekly Target row to "Serves which goals?" → tap the goal pill; then confirm the next evaluation reports `tier 1` and a non-empty `matched_via`. |
...
| 54 | **L2 — arc Tier-2 matching** | **The arc's keyword tier carries the exact defects #48 measured and fixed elsewhere** | **✅ CLOSED — PORTED AND VERIFIED (2026-08-04, session #49), `b55c627`** | `getGoalSessionDates` is REMOVED; the arc calls the shared `hxMatchGoalDays()`, reading `specific_dates` (not the raw category set — that would have credited a bench goal 45 strength sessions and a running goal 25 cardio sessions with zero runs). Measured row by row on profile 1's real log: **7 false positives removed** (bench 3 × `Overhead Press`, half-marathon 4 × crunches), **1 true positive gained** (`Pull-Up`), **zero** names added or removed across all 8 real goal titles × 75 names, and all three migrated goals' arc states **byte-identical** (`position 0 / calendar 1 / drift −1 / on_track / qualifying_28d 0`). `tier`/`confidence` labels unchanged; `matched_via` now populated. **REMAINING FOLLOW-UPS, both deliberately out of scope:** (a) `targetServesGoal` (Layer 3 schedule links) still uses `extractGoalKeywords` — it decides links, not evidence, and matters more once linking is actually used; (b) a SAFE rule for using a goal DESCRIPTION, which is currently excluded because tokenising freeform prose reached `Thoracic Extension Over Foam Roller` through the preposition `over`. |
| 55 | **L2 — arc evidence rule** | **The arc counts only days that NAME the goal, never raw category agreement** | **✅ SHIPPED + VERIFIED (2026-08-04, session #49)** | `arcKeywordDates` returns `hxMatchGoalDays().specific_dates` — exercise-level matches PLUS category days whose own text or exercises mention a specific goal term. Proven both ways on the seeded runner pair reproduced in test: a real runner earns **21** days including a cardio session carrying **zero exercise rows** (the reason the category layer is mandatory), while a non-runner with the SAME 21 cardio sessions earns **0**. Regression-tested against the `other`-category phantom case: "Fix Posture" / "Fix Pubic Osteitis" / "Build Muscle" each match **0** uncategorised workouts, and the guard is proven non-vacuous by bypassing it in-sandbox and observing the 4 phantom days return. |
| 56 | **Roadmap grounding** | **A goal roadmap is no longer grounded in a different exercise** | **✅ SHIPPED + VERIFIED (2026-08-04, session #49)** | `getGoalExerciseContext` takes token objects and the exercise layer ONLY. Measured on profile 1's real rows: a bench goal went from `total_sessions 2, best_set 12.5 lb × 10, trend plateauing` (Overhead Press) to the honest zero shape; a running goal went from `total_sessions 2, best_set 5 lb × 25` (crunches) to the same. A category-only match is asserted to produce the exact zero object — never a null-filled one that reads as evidence — and a REAL match still grounds correctly, so the change is not a mute button. All 5 call sites adapted; the per-goal aggregation loop is untouched. |
| 57 | **Blast radius** | **The daily-rec prompt is provably outside this change** | **✅ VERIFIED (2026-08-04, session #49)** | `public/index.html` is **byte-identical** to the pinned pre-change commit `aff60c1` (`git diff --quiet`), and contains **0** references to `extractGoalKeywords` / `getGoalExerciseContext` / `hxGoalTokens` / `arcKeywordDates`. The rec prompt is assembled in the browser, so the char delta is **0 by construction** — stated rather than re-measured in a browser, per the session brief. |
...
1. ~~**BUG 1 is NOT migration-caused, and migration does not meaningfully worsen it.**~~
   **✅ SUPERSEDED 2026-07-28 (session #43): BUG 1 IS FIXED, AND PAYLOAD SIZE IS NO LONGER A
   MIGRATION CONSIDERATION AT ALL.** **Both steps shipped** (commit `898fd02`): step 1 moved
   `exercises` + `workoutLog` server-side, step 2 moved `goals` server-side via
   `loadProfileWithGoals()`. The `/goal-progress` request body went **207,353 B → 86 B**, and
   because **no goal data is posted any more**, the measured `+8,653 B` migration delta on
   `profile_data.goals` now adds **exactly 0 bytes** to that request. The endpoint is permanently
   immune both to history growth and to goal-shape growth. **This input is now closed — it neither
   argues for nor against migrating.** (Session #42's original reasoning, retained for the record:
   the body was 2.02× the limit *today*, accumulated `profile_data.goals` was only 18.1% of it and
   under the limit on its own, and the client blob at 81.9% was already 1.66× on its own — so
   migration was never the cause and not migrating was never the fix. Full numbers: §6 → "BUG 1 —
   RESOLVED" and the AUDIT RESULT block beneath it; §7 ledger row 29.)
...
**As of session #43 there is exactly ONE cost left on the table: cost 2.** Cost 1 (overwriting real
adaptation history) is unchanged and still real. The two *blockers* people worried about are both
gone — the arc-wipe-on-adapt (BUG 3, fixed session #42) and the payload concern (BUG 1, fixed
session #43).
...
##### ✅ A1 ANSWERED — 2026-08-02, session #46: CAN THE MIGRATION BE SCOPED TO ROADMAPS ONLY?

**The athlete's question, verbatim intent: migrate how roadmaps work WITHOUT changing the daily
workout-suggestion format, because the suggestion format on the sandbox profile read badly.**

**Answer: the two are ALREADY separable, because rec FORMAT does not read goal shape at all.
Migration changes rec CONTENT, not rec FORMAT — and the sandbox evidence does not support the
concern in the first place.**

**1. What migrating profile 1's goals actually turns on.** Migration means giving each goal
`goal_type` + `demand` + `estimate`, which makes `derivePhasePlan()` derive the phase count and each
phase's week budget, which makes the goal **arc-eligible** (`evaluateArcsForProfile` filters on
`roadmap.estimate` + a `near_term` phase), which produces `roadmap.arc_origin` + `roadmap.arc_state`,
which switches `recomputeRoadmapProgress` from the time-elapsed branch to the earned-position branch.
Layer 3 (`capacity`, `coexistence`) is a **separate** opt-in captured at intake and is not implied.

**2. Which of those inject anything into the DAILY REC prompt — the crux. Exactly ONE.**

| new state | reaches the daily-rec prompt? | via |
|---|---|---|
| `goal.goal_type` / `goal.demand` / `goal.estimate` | **no** | nothing in `fetchAI` reads them |
| derived phase count / `duration_weeks` | **no** | phase *content* already reaches it via `buildRoadmapEmphasisContext()`, which works on legacy roadmaps today (**1,572 chars on profile 1 right now**) |
| `roadmap.arc_state` / `arc_origin` | **YES — the only one** | `buildArcStateContext()`, one block, top-3 goals, self-capping at `ARC_CONTEXT_CHAR_CAP = 950` |
| `profile_data.capacity` | **no** | grep-verified: read only by the Profile-tab capacity card and the goal-roadmap view |
| `profile_data.coexistence` | **no** | grep-verified: read only by the Profile-tab coexistence card |

**3. Rec FORMAT is governed by four things, none of which read goal shape.**
`buildResponseShapeSpec(aiNumOpts, aiShowMV, durations)` (the JSON skeleton and section rules),
`buildTimeBudgetContext(durations, intensity)` (the time budget + the Layer-4 depth floor),
`REC_DEPTH_TIERS` / `recDepthFloorFor()` (movements per section), and
`resolveOptionDurations()` (the duration ladder, from the schedule and the athlete's pill). **Their
signatures take durations, counts and settings — not goals.** So a migrated profile 1 gets the
**identical card shape, identical section structure, identical depth floor and identical duration
ladder**. What changes is that the model is additionally told where the athlete has actually *earned*
their way to, and instructed to prescribe for that position rather than the calendar.

**Verdict: SEPARABLE — and the separation needs no flag, no gate and no injection point, because it
already exists.** `buildArcStateContext()` returns `''` for any goal with no `arc_state`, so it is
already per-goal opt-in. If the athlete wanted the arc *computed* but not *narrated*, the mechanism
is a one-line early return in that one function — but that would discard the entire athlete-facing
payoff of Layer 2, so it is **not recommended**.

**4. WAS the sandbox rec format actually worse? Checked independently — and the comparison is
invalid.** Profile 9 has **0 workouts, 0 exercise rows, 0 coaching brief, 0 historical brief, 0
micro-goals, no wearable, and no `daily_recommendations` row at all** (so the rec the athlete judged
is not even recoverable). Any rec generated there was assembled from the system prompt, three goal
titles, one roadmap's emphasis, and an arc block reading week 0 — with **no history to be specific
about**. A thin, generic rec is the correct output of that input. **This is thin/synthetic history,
not a Layer 4 regression and not the negotiation-corrupted goal state #45 fixed.** The concern does
not survive #45 and does not transfer to profile 1, which has 92 workouts and 326 exercise rows.

**5. ⚠ ONE REAL PREREQUISITE, and it is new.** Profile 1's cold-boot rec currently ships with
**`exerciseHistory` = 0 chars** because `bootApp()` never calls `loadLibrary()` (§6, session #46).
Migrating on top of that would add an arc block that says *"prescribe for the EARNED position"* to a
prompt that **cannot see what the athlete has actually lifted**. That is the worst of both worlds:
more instruction, no more evidence.

**RECOMMENDATION (one, as asked): fix the `loadLibrary()` race first, then migrate all three
roadmaps in one pass — and do NOT scope the migration to "roadmaps only", because there is nothing
to scope out.** The format concern is answered by the table above; the remaining cost is unchanged
and is still cost 1 (regeneration overwrites three roadmaps carrying real `adaptation_log` history —
use the Coach-Chat regen path, which preserves `version`/`adaptation_log`, not the reset path).

---

##### ✅ A2 ANSWERED — 2026-08-02, session #46: DOES THE BRAIN SEE THE WHOLE HISTORY?

**The athlete's stated expectation: the system should analyse the ENTIRE workout history to know
where he stands against a roadmap.**

**Every relevant consumer's real window, measured from the source:**

| consumer | workouts | exercises | verdict |
|---|---|---|---|
| **Arc / earned position** (`evaluateArcsForProfile`, `computeArcState`) | `date >= arc_origin`, **`limit=2000`** | `date >= arc_origin`, **`limit=8000`** | **✅ full history from the origin — by design, and the fetch window is deliberately keyed off `arc_origin` and NOT off phase dates (session #37 bug #2)** |
| **Weekly adapt** (`maybeAdaptAllRoadmaps`) | **`limit=10`**, `select=date,type` | `getFullExerciseContext(60)` + per-goal `getGoalExerciseContext(90)`, each `limit=5000` | ⚠ **10 workout rows ≈ 12 days at this athlete's rate** |
| **Goal progress** (`POST /goal-progress`) | **`limit=60`**, `ts.desc` | all-time, `limit=5000` | ⚠ 60 workouts ≈ **11 weeks** |
| **Daily rec prompt** (`fetchAI`) | `workoutLog` = **`limit=60`** (weekly volume, variety, streak) | **7-day slice** of `libExercises` + top-10 by session count for PRs | ⚠ and today it is **0** on a cold boot (§6) |
| **Coaching brief / historical brief / history search / analytics** | `limit=10000` (all-time) | `limit=10000` (all-time) | ✅ full history |
| **Macro roadmap generate** | `limit=30` | `getFullExerciseContext(90)` | — |
| **Per-goal roadmap generate** | `limit=20` | goal + full context | — |

**FLAGGED HARD, as asked: earned-position computation is NOT capped — but the adapt is, and the
daily rec is.** The arc replay is the one place that genuinely reads everything, and it is also the
one place the "code computes, model narrates" invariant is fully honoured. **The 60-row cap is the
one to quantify:** profile 1 has logged **92 sessions between 2026-04-07 and 2026-08-02 — 118 days,
≈ 5.5 sessions/week.** At that rate **60 rows covers ~11 weeks**, and it is measurably *already*
binding: the top 60 by `ts` reaches back to **2026-05-09**, so 32 real sessions (all of April plus
8 days of May) are outside every `workoutLog`-driven surface. For a heavier logger — 7/week — 60
rows is **8.5 weeks**.

**The architectural correction, stated explicitly for the docs:** the ANALYSIS should cover full
history, but **per the standing invariant it is CODE that analyses and the model that narrates a
code-computed result — the model is never handed raw history to interpret.** Current behaviour
**honours this where it counts and violates it nowhere**: `computeArcState` replays the entire log in
code and the AI only reads the resulting one-line `ARC STATE` block; `getFullExerciseContext` /
`getGoalExerciseContext` aggregate in Node and hand the model rollups, not rows; the coaching and
historical briefs are the one place the model *does* see a wide window, and they are explicitly a
**summarisation** step whose output is then treated as text. **The gap is not the invariant — it is
that some code-side analyses are handed a truncated window to analyse.**

**PROPOSED (do not build), with cost:**
1. **Aggregate, don't widen.** Add a server-side `getTrainingHistorySummary(profileId)` that reads
   all-time `workouts` + `exercises` (both already indexed by `profile_id`) and returns
   **per-month category counts, per-exercise first/last/best, longest gap, and total sessions** —
   a few hundred chars of rollup regardless of history size. Feed it to the adapt and the daily rec
   in place of raw rows. *Cost: one new helper (~80 lines), one Supabase read per consumer, ~300–500
   prompt chars. No schema change.* **This is the right shape — it scales to 10 years.**
2. **Raise the adapt's workout window 10 → ~40** so `buildWeeklyReviewContext`'s own 14/28-day
   buckets are actually populated. *Cost: one number. No prompt-size change (it emits counts).*
3. **Leave the arc alone.** It already reads everything and is the model of how the rest should work.
4. **Do NOT raise `loadWorkouts()`'s 60 to "all".** That is the display problem, and the answer there
   is pagination/date-range (A3), not a bigger blob — session #43's standing rule about
   `/goal-progress` is the precedent.

---

##### ✅ A3 ANSWERED — 2026-08-02, session #46: "WORKOUT HISTORY SHOULD LIVE FOREVER"

> **RECORDED AS A PRODUCT PRINCIPLE. Nothing in the codebase deletes history unattended (§6 has the
> full enumeration of every delete/overwrite path). Every cap below is a READ cap, not a retention
> policy — the data is all still there.**

**Every place history is capped, truncated, paginated or dropped:**

| # | where | cap | correct or wrong |
|---|---|---|---|
| 1 | `loadWorkouts()` → `workoutLog` | **60 rows** | **⚠ WRONG — this is the one the athlete hit.** It is the sole source for the History list, the calendar, the streak, and the rec prompt's weekly-volume/variety blocks. A user cannot view their own past training. |
| 2 | Log-past panel `lpFetchNextPage()` | pages at 60 via `?offset=` | ✅ correct — and it is the *only* surface that can reach April today |
| 3 | `buildLog()` (rec prompt) | last **7 days**, trimmable to 4 | ✅ correct — prompt budget |
| 4 | `buildExerciseHistory()` (rec prompt) | top **10** by session count, trimmable to 5 (+ hold-PR re-attach) | ✅ correct — prompt budget |
| 5 | `maybeAdaptAllRoadmaps` | **10** workouts | ⚠ wrong-ish — see A2 item 2 |
| 6 | `POST /goal-progress` | **60** workouts | ⚠ borderline — replicated deliberately in #43 to preserve byte-identity; should follow #1 |
| 7 | `getFullExerciseContext` | **60 days** | ✅ correct — a rolling recency window by design |
| 8 | `getGoalExerciseContext` | **90 days** | ✅ correct |
| 9 | `GET /api/meditations` | 30 rows | ⚠ wrong for a habit surface, harmless today |
| 10 | `daily_steps` history endpoint | clamps to **365 days** | ⚠ known (session #33) — the table holds 736 rows |
| 11 | `daily_sleep` | **no listing endpoint exists at all** | ⚠ known (session #33) |
| 12 | Coach Chat snapshot | 7-day exercise log + **30-day** sleep block | ✅ correct — hard char guard; the all-history sleep rollup is already a decided-to-build item |
| 13 | arc replay | `arc_origin` → today | ✅ correct |
| 14 | `historical_brief` | regenerated on demand only | ⚠ **stale: `2026-07-15`**, and it is currently carrying strength context by accident (§6) |

**PROPOSED (do not build) — what permanent, fully viewable history requires:**
1. **Date-range + pagination on the History tab**, not a bigger `limit`. The server already accepts
   `?offset=`; the missing pieces are a month/year picker and a `?start_date=`/`?end_date=` pass-through
   on `GET /api/workouts`. *Cost: small server change (two query params), medium client change
   (calendar already knows about months). This is the actual fix for the athlete's complaint.*
2. **Aggregate strategy so prompts stay in budget** — A2 item 1. Full-history *analysis* must never
   mean full-history *rows in a prompt*. Today's measured headroom is 947 chars warm; a rollup block
   fits, a row dump does not.
3. **Retention/backfill gaps still outstanding:**
   - **The Fitbit → Google Health historical activity backfill is NOT built** (§7 near-term item 2).
     `backfill-wearable-history` is **Fitbit-only**, and the **Fitbit Web API shuts down September
     2026**. Sleep/HRV/steps history was backfilled 2026-07-17 (675/674/653 rows) and RHR was
     re-runnable after the chunking fix — but **activities** (the workout-matching source) have no
     GH equivalent. After the shutdown there is no remaining source for any further gap-filling.
   - **8 malformed-`date` workout rows** are effectively invisible history (§6) and the write path is
     still open.
   - **Historical `daily_sleep` / `daily_steps` `source` labels before 2026-07-20 are permanently
     wrong** and cannot be corrected (§6, session #31).
   - **`workouts` has no `created_at`/`updated_at`**, so "when was this row really written" is
     unanswerable — which is what makes repairing the malformed-date rows a review job.
...
| **Post-arc bugs** | ✅ **2026-07-27 (session #42):** BUG 2 diagnosed as NOT a bug; BUG 3 found and fixed (`ae46a96`); BUG 1 fully audited. ✅ **2026-07-28 (session #43):** BUG 1 fixed and verified live (`898fd02`); ledger row 22 closed. |

> **⚠ THE BUG QUEUE IS NOW EMPTY. Item (c) (ledger row 28) is the last unproven link in the
> Layer 2 → Layer 4 chain** — a re-ramp the athlete cannot *see* in their session is a re-ramp the
> arc did not deliver. **It is no longer blocked on code or on any other row: route (1) was chosen
> by decision, so it is waiting on real accumulated data only.** The former ordering
> (BUG 2 → BUG 1 → item (c)) is discharged: BUG 2 was not a bug (session #42), BUG 1 shipped
> (session #43), and row 22 is closed (session #43).
...
### Added 2026-08-04 (session #49)

- **⚠ `exercises.duration_minutes` OVERLOAD — FOURTH sighting. Now formally accruing debt with a
  scoped proposal. NOT BUILT — this is a record, not a task.** The column means **both** a hold
  duration (Dead Hang `0.5` = 30 s) **and** a session length (a 28-minute run = `28`), with nothing
  in the schema distinguishing them. Every feature that needs to render or reason about it has had
  to re-implement the same category heuristic:
  1. **Engine v2 `v2Progression.js`** — disambiguates by `main_category` into `best_duration_seconds`
     vs `best_session_minutes` (found by running the audit against real data: it had reported
     *"MMA Sparring PB 60:00 hold"* and prescribed `+5-10 s hold` on a sparring session).
  2. **Session #48 `renderTrainingHistoryBlock`** — `HX_SESSION_LENGTH_CATEGORIES`, after a 28-minute
     run rendered as **"28:00 hold"**.
  3. **Session #49** — inherits (2) unchanged through the shared matcher.
  **Three separate features, one workaround, copied three times.** The heuristic is also only ever
  correct on average: a 45-minute yoga session and a 45-second yoga hold are indistinguishable.
  **Scoped proposal (do NOT build without its own session):** add a nullable
  `exercises.duration_kind text CHECK (duration_kind IN ('hold','session'))`, defaulted `null` =
  "unknown, fall back to the existing category heuristic", so every existing row and every existing
  reader keeps working byte-identically. Populate it forward from the extraction prompt (which
  already knows which it emitted) and leave history unconverted rather than guessing. Retire the
  three copies of the heuristic onto one helper that reads the column when present. **Cost is one
  migration + one extraction-prompt field + one shared helper; the risk is that a
  partially-populated column is worse than a consistent heuristic, which is exactly why it needs its
  own scope and its own before/after.**
- **`targetServesGoal` (Layer 3 coexistence) still uses `extractGoalKeywords` + raw `indexOf`.** It
  is now the ONLY consumer of that function. It decides schedule LINKS, not evidence, so it was left
  untouched by decision — but it carries the same defects (`run` substring-matches `Crunches`,
  hyphen asymmetry, generic-token overlap) and it matters **more** once linking is actually used,
  which is exactly what ledger row 50 is asking the athlete to do. §7 ledger row 54.
- **A goal DESCRIPTION is currently unusable as matching input, and a real signal is being given up
  for it.** Tokenising freeform prose reached `Thoracic Extension Over Foam Roller` through the
  preposition `over`. But `Fix Posture`'s description genuinely reaches `Shoulder Blade Retraction` /
  `Shoulder Blade Mobility` / `Shoulder Roll` through "rounded shoulders". **Candidate rules, none
  built:** admit only description tokens that appear in the `exercise_catalog` / logged-name
  vocabulary; or cap description tokens to the first N content words; or require a description token
  to co-occur with a title token. Needs its own before/after on real goals.
- **`shoulder` is a candidate `HX_GENERIC_TOKEN` and was NOT added.** It is an anatomical region in
  the same class as `leg` / `arm` / `core` / `upper` / `lower`, which ARE generic. Adding it would
  change #48's shipped intake behaviour, so it was left alone. Worth deciding deliberately rather
  than by omission.
- **Harness: `stripToCode()` blanks string bodies by design** — searching its output for a string
  literal can never match, and the failure reads like a product regression. Fired **twice** in
  `arcMatcherPort.test.js`. Convention going forward: **raw slice for literal checks, stripped slice
  for identifier/call checks**, and say which in a comment.
- **Two pre-existing suites had extractors coupled to source details this session changed** —
  `historyIntake.test.js`'s slice list (a refactor moved functions out from under it) and
  `arcFlag.test.js`'s end marker (a **comment** it sliced up to was rewritten). Both were fixed
  without weakening an assertion. **Convention: a slice end-marker should be a DECLARATION, never a
  comment.**
...
- [ ] **⚠ `bootApp()` never calls `loadLibrary()`, so every cold-boot daily rec is generated with an
  EMPTY exercise history — the highest-value item in this list.** Measured: `libExercises = 0` after
  a full cold boot; `exerciseHistory` 3,548 → **0** chars, `recentLog` 765 → **39** chars (the literal
  false line *"No exercises logged in the last 7 days."*). §6 has the full table. **This is the second
  instance of the race session #43 fixed for `/goal-progress`, and §9 explicitly predicted more
  existed.** **Three candidate fixes, in ascending cost:** (a) add `loadLibrary()` to `bootApp()` —
  one line, but it adds two GETs to the boot critical path and does not remove the race, it just
  makes it usually-win; (b) have `resolveAIRecs()`/`fetchAI()` **await** an exercise fetch the way
  `fetchAI` already awaits `microGoalsReady` — deterministic, and the pattern is already in the
  function; (c) move both builders server-side, the full session-#43 treatment. **(b) is the shape
  that matches the existing code.** **Also fix the honesty bug regardless of which is chosen:**
  `buildLog()` should not emit *"No exercises logged in the last N days"* when `libExercises` is
  simply unloaded — an unknown must not be reported as a zero, and today it directly contradicts the
  `THIS WEEK SO FAR` block in the same prompt.
- [ ] **Audit the remaining `libExercises` / `workoutLog` read sites for the same race.** Session #43
  logged this as unaudited; session #46 confirmed one live instance in the highest-traffic consumer.
  Known readers not yet checked: `renderLibExercisesGrouped`, `buildLoggedCountIndex`,
  `updateLibImportBanner` (`index.html:15029`), `ensureHistoryChipsLoaded`, and every
  `calculateStreak`/weekly-volume path that assumes `workoutLog` is populated.
- [ ] **⚠ The malformed-`workouts.date` write path has never been traced, and it is STILL PRODUCING
  ROWS.** 8 rows now (was 6 as of session #33); ids **100143 / 100144 were written 2026-07-29,
  three seconds apart**. `workouts.date` is `text`, `POST /api/workouts` forwards `req.body`
  verbatim, and the future-date guard only engages when `body.date` parses — so a time string passes
  straight through. **First step is finding the client path that sends a time string as `date`**
  (suspects: `reLogWorkout` / `useTemplate` prefill, the past-date picker, or a wearable-import
  path), then a `CHECK (date ~ '^\d{4}-\d{2}-\d{2}$')` constraint or a server-side validation.
  **Repairing the 8 existing rows is a separate decision** — each row's real date is recoverable from
  `ts`, but `ts` is a last-write stamp (§6), so this is a review-then-apply job, not an auto-fix.
- [ ] **`POST /api/profiles/:id/dedupe-workouts` performs a BULK DELETE from a single unconfirmed
  tap, and is not admin-gated.** `dedupeWorkouts()` (`index.html:15533`) has no `confirm()`, unlike
  its neighbour `reformatAllTitles()`. The endpoint groups on `date + '|' + ts` and deletes every
  row after the first in each group. Low real-world risk today (`ts` is `Date.now()`), but it is the
  only unconfirmed destructive control in the app, and the malformed-date rows above are exactly the
  class of row most likely to collide. **Add a confirm at minimum; consider a dry-run report first,
  matching the orphaned-exercises pattern.**
- [ ] **`POST /api/profiles/:id/reformat-titles` overwrites `workouts.type` on EVERY workout with
  notes** (limit 10000, one AI call each, `temperature` unpinned on this call path since it uses bare
  `callAI(prompt, 100)`). `workouts.type` is what `inferWorkoutCategory()` reads for every weekly
  target count, every category tally, and the variety block — so a re-run silently re-shapes the
  athlete's whole category history. Confirmed behind a `confirm()`, but there is **no dry run, no
  before/after report, and no way back**. Same discipline as the exercise-canonicalization backfill
  (report → review → apply) would apply cleanly here.
- [ ] **`profile_data.schedule.frequency_targets` has no priority weighting and no
  category-uniqueness check.** Two targets can resolve to the same inferred category (profile 1:
  "Upper Body Strength" and "Lower Body Stength" both → `strength`), which makes
  `schedTargetDoneCount` count the same workouts for both and lets one session clear two targets;
  and the Option-1 scorer uses raw deficit only, so a 2×/week target always outranks a 1×/week target.
  §6 has the measured 14-day consequence. **This is a design question, not a patch** — options are:
  score by `deficit / times_per_week` (proportional), let a target declare a goal link and weight by
  goal priority (the keystone join already exists), or surface the collision in the Schedule editor.
- [ ] **The Living Goal Roadmap card cannot show that an adapt ran.** The adapt writes genuinely new
  analysis every week and it all lands in `adaptation_log`, which the card collapses behind
  "Show history (N updates)". Meanwhile the visible surface (progress bar, phase status) is
  **date-derived** on a legacy roadmap and moves ~1.8 %/day. **Cheapest honest fix: surface
  `last_adapted_at` and the newest `adaptation_log` summary uncollapsed on the card**, so "your coach
  reviewed this on July 30" is visible without a tap. Migration fixes the progress-bar half of this
  (earned position replaces time-elapsed); it does not fix the log visibility.
- [ ] **`maybeAdaptAllRoadmaps` builds its weekly review from `limit=10` workouts.** At ~5.5
  sessions/week that is ~12 days, so `buildWeeklyReviewContext`'s 14-day and prior-14-day buckets are
  built from a window that barely covers the first one. Widening to ~40 rows would cost one larger
  Supabase read and no extra model tokens (the function emits counts, not rows).
...
- [x] **✅ BUG 1 — goal-progress 413s. FIXED AND VERIFIED LIVE 2026-07-28 (session #43), commit
  `898fd02`. Both steps shipped in one session; body 207,353 B → 86 B (99.96%).** No migration, no
  schema change, no new endpoint, **no limit bump**. Step 1 moved `exercises` + `workoutLog`
  server-side (windows replicated exactly: workouts `select=*`/`ts.desc`/60 — the client's
  `.slice(0,90)` never bound, since `loadWorkouts()` caps at 60; exercises `select=*`/`date.desc`/
  `limit=5000`, all-time, and the client's array was that endpoint's grouped `sessions[]`
  flattened, which are the raw rows verbatim). Step 2 moved `goals` server-side via
  `loadProfileWithGoals()`. **The per-goal aggregation loop was not touched.** Verified before
  deploy by booting the REAL pre-fix and post-fix servers with `node-fetch` stubbed and exercising
  the actual route against the same real read-only profile-1 fixture: pre-fix reproduced the 413 +
  `<!DOCTYPE` SyntaxError verbatim; **response byte-identical for all 8 goals** (sha
  `f5077b6076893503`); **all 5 AI prompts byte-identical** (sha `287284ee51079508`); zero Supabase
  writes both paths; stale-client and Supabase-outage paths both hardened. Suites 213/213. Verified
  live: 86-byte body → 200, 3 deterministic goals byte-identical to the baseline, `profile_data`
  sha unchanged across the POST. **STANDING RULE: never raise the body limit for this endpoint, in
  any form** — it is a stopgap against a payload that grows with every logged row, and the body is
  now 86 bytes and migration-proof. Full detail: §6 → "BUG 1 — RESOLVED"; ledger row 29. Original
  entry and the session-#42 audit are retained as the basis:
- [ ] **⚠ NEW, session #43 — `fetchGoalProgress()` used to race the client's own loaders, and it
  was silently sending EMPTY arrays on a cold boot.** `renderProfileGoals()` is called by
  `bootApp()` via `renderProfileTab()`, but **`bootApp()` never calls `loadLibrary()` at all**
  (it is only called on a Library tab switch, `public/index.html:17973`), and `loadWorkouts()` is
  still in flight at that moment. So before the fix, a cold-boot `POST /goal-progress` frequently
  arrived with `exercises: []` and `workoutLog: []`, computing strength / distance / consistency
  goals against no data — meaning the pre-fix results were **non-deterministic, contingent on
  whether the athlete had visited the Library tab in that session**. The session-#43 fix removes
  the race entirely (the server fetches its own rows), so this is **already resolved for this
  endpoint**. It is logged because **the same race shape may exist at other `libExercises` /
  `workoutLog` read sites reached from the boot path** — never audited. Cheap next step: grep every
  reader of those two globals and check which are reachable before their loader resolves.
- [ ] **Superseded BUG 1 entry, retained for the record (opened session #41, audited session #42):**
...
- [ ] **⚠ NEW, session #43 — a jsonb round-trip REORDERS object keys, so a `JSON.stringify` diff
  ...
  failed, purely on key order. **Any future before/after or replay-vs-live comparison against a
  jsonb column must canonicalise key order first** (sort keys recursively) — or it will report a
  difference that does not exist. Recorded so it is not re-debugged; the same trap applies to the
  sha256 comparisons this project uses on `profile_data` (those are safe only because both sides
  come from the same source in the same order).
- [ ] **The bench goal's destroyed `arc_origin` is NOT recoverable (residual of BUG 3, session
  #42). ⚠ UPDATED session #43 — route (1) chosen by decision; the goal now has a PROTECTED origin
  going forward.** The 2026-07-28 arc evaluation (row 22) caused the bench goal to re-acquire an
  `arc_state` (`position_week 0, calendar_week 2, drift −2, confidence precise, tier 1`) with its
  origin pinned at the **2026-07-26 regeneration date** — the BUG 3 residual materialising exactly
  as predicted. **From 2026-07-26 forward the session-#42 fix guarantees the origin no longer
  walks**, so real qualifying sessions can accumulate toward a genuine peak-then-gap. The lost
  pre-2026-07-26 earned weeks remain unrecoverable and **were not guessed**. Original entry: Checked every field that could carry it — `roadmap` keys, `goal` keys, all 7
