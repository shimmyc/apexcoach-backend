...
// GET /api/debug/source-constraint-probe/:userId?secret=ADMIN_SECRET
//
// Gate for threading a real provider into `source`. These upserts are
// fire-and-forget, so if a CHECK constraint rejects "google_health" the write
// fails and the row is LOST with only a console line — the exact shape of the
// duration_minutes data loss (session #30). The committed DDL declares
// `source text default '...'` with no CHECK, but this project has a history of
// constraints added by hand outside migrations (chat_proposals.type, found
// only via a live 23514), so the live state is verified rather than assumed.
//
// Writes ONLY to the throwaway date 1970-01-01 and deletes both rows again.
app.get("/api/debug/source-constraint-probe/:userId", async function(req, res) {
  if (process.env.ADMIN_SECRET) {
    var gotScp = req.query.secret || req.headers["x-admin-secret"];
    if (gotScp !== process.env.ADMIN_SECRET) return res.status(403).json({ success: false, error: "forbidden" });
  }
  var pid = parseInt(req.params.userId, 10);
  var testDate = "1970-01-01";
  var out = {};
  async function tryWrite(table, extra) {
    var scope = "profile_id=eq." + pid + "&date=eq." + testDate;
    var payload = Object.assign({ profile_id: pid, date: testDate, source: "google_health" }, extra);
    var result = { accepted: null, status: null, error: null, stored_source: null };
    try {
      var r = await fetch(SUPABASE_URL + "/rest/v1/" + table + "?on_conflict=profile_id,date", {
        method: "POST",
        headers: sbHeaders("return=minimal,resolution=merge-duplicates"),
        body: JSON.stringify(payload),
      });
      result.status = r.status;
      result.accepted = r.ok;
      if (!r.ok) result.error = (await r.text()).substring(0, 300);
      else {
        var v = await (await fetch(SUPABASE_URL + "/rest/v1/" + table + "?" + scope + "&select=source", { headers: sbHeaders() })).json();
        result.stored_source = Array.isArray(v) && v[0] ? v[0].source : null;
      }
    } catch (e) {
      result.accepted = false;
      result.error = String(e.message || e).substring(0, 300);
    }
    // Always clean up, even on failure.
    try { await fetch(SUPABASE_URL + "/rest/v1/" + table + "?" + scope, { method: "DELETE", headers: sbHeaders("return=minimal") }); } catch (e) {}
    return result;
  }
  try {
    out.daily_sleep = await tryWrite("daily_sleep", { hours: 1, score: 1 });
    out.daily_steps = await tryWrite("daily_steps", { steps: 1 });
    var allOk = out.daily_sleep.accepted && out.daily_steps.accepted;
    res.json({
      success: true,
      profile_id: pid,
      test_date: testDate,
      results: out,
      verdict: allOk
        ? 'Both tables ACCEPT source="google_health" — no CHECK constraint blocks it. Safe to thread the real provider through.'
        : 'At least one table REJECTED source="google_health". Do NOT thread the value until a migration widens the constraint — a rejected fire-and-forget write loses the row.',
      cleaned_up: true,
    });
  } catch (e) {
    console.error("[source-constraint-probe] failed:", e.message);
    res.status(500).json({ success: false, error: e.message });
  }
});
