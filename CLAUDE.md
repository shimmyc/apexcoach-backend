...
- **Readiness card left monolithic, not split** — `renderReadiness()` was audited and found to render one long `card.innerHTML` (hero ring + bio grid + HRV/sleep/RHR bars + sleep-stage detail incl. the computed sleep score + zone minutes + HRV stat cards) with no seam to cleanly separate a compact "above-fold hero" from the detail below without a JS restructure. Accepted as-is for this pass — see ROADMAP.md §7 for the deferred follow-up.
