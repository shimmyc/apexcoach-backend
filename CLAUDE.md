...
**Deliberately deferred** (found during the audit, same bug class, lower severity or higher cost to fix — reported before any code was written, confirmed with the user): analytics current/longest streak (`currentStreakFromDates`/`longestStreakFromDates`) and weekly-volume/most-active-day bucketing in exercise insights — **the agreed immediate follow-up task after this ships**; roadmap phase date assignment (`assignNearTermDates`) and the `getGoalExerciseContext`/`getFullExerciseContext` 60-90 day rolling windows — a 1-day skew is invisible at multi-week/90-day granularity; `life-os-summary`'s own date-param override behavior for its external caller — already has its own `?date=` override, different consumer/conventions; `POST /api/profiles/:id/daily-recs`'s `fallbackDate` — the primary path is already client-supplied (athlete-local via the browser's `ds(0)`), this only matters if the client omits `date` entirely.
...
**Schema** (migration `migrations/2026-07-15_chat_proposals.sql`, run manually):
- `chat_proposals` — `id`, `thread_id` (FK), `message_id` (FK, nullable — backfilled after the assistant message is saved post-stream, since a proposal is created mid-stream before that row exists), `tool_use_id`, `type` (`update_goal`\|`set_focus_override`\|`log_checkin_note`), `payload` (jsonb — `{title, changes:[{field,label,before,after}], reason, ...type-specific internal fields}`), `status` (`pending`\|`confirmed`\|`canceled`), `created_at`, `resolved_at`. RLS + `service_role_bypass`, matching every other table.
...
**Not built: roadmap-regenerate offer after a confirmed goal update.** Verified 2026-07-15 (documentation-sync session) — grepped `server.js` for any "offer to regenerate" instruction in `CHAT_SYSTEM_PERSONA`/`COACH_CHAT_TOOLS`/`applyProposal()` and found none. A confirmed `propose_goal_update` today silently leaves any existing per-goal (`goal.roadmap`) or macro (`roadmap_data`) roadmap stale until the next weekly `maybeAdaptAllRoadmaps()` pass or a manual Regenerate. Small pending item — see ROADMAP.md §6.
