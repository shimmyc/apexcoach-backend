...
A 5-agent UI overhaul is underway. **Agent 1/5 (token foundation)**, **Agent 2/5 (Today tab)**, **Agent 3/5 (bottom nav + modals)**, and **Agent 4/5 (History + Library tabs)** are implemented. The work is additive — the legacy `--bg`/`--accent`/`--text` vars and the 10-theme system above still drive the un-migrated **Profile** tab and are untouched. Agent 5 migrates Profile + Settings content.
