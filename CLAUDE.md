...
- Today tab: Fitbit biometrics + readiness score + progress brief + 3 AI workout options. **DOM order (2026-07-16 declutter pass, see "Today Tab + Profile Tab Reorganization" below)**: date/day-nav → conditional banners/status/manual-checkin gate (unchanged) → `#readiness-card` → `#feeling-checkin-card` → `#ai-card` — nothing else between readiness and the rec — then below the fold `#progress-card` (Cornerman) → `#unmatched-fitbit-card` → `#streak-card` → `#log-area`.
...
### Drop `fitbit_pending_imports` (2026-07-17) — ⚠ not yet run
...
### Drop legacy text roadmap columns (2026-07-17) — ⚠ not yet run
...
### `exercises.workout_id` FK cascade (2026-07-17) — ⚠ not yet run, orphan check required first
Adds a real FK from `exercises.workout_id` to `workouts.id` with `ON DELETE CASCADE`. **Will fail if any `exercises.workout_id` doesn't match an existing `workouts.id`** — run the orphan report (`GET /api/debug/orphaned-exercises/:userId`) for every profile first (profile 1 was cleaned in session #11; profiles 4/5/7/8 have not been checked for this). Does not affect rows where `workout_id IS NULL` — `extract-exercises` can insert one, and NULL FK values are always valid. See `migrations/2026-07-17_exercises_workout_fk_cascade.sql`.
