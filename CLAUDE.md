...
- **Migration** `migrations/2026-07-18_exercise_catalog_variation_group.sql` (**run manually — not yet applied**): adds `variation_group text` (nullable) + a partial index (`WHERE variation_group IS NOT NULL`). Column-add only, no RLS/policy change.
- **`POST /api/debug/seed-exercise-variations`** (admin-gated, `?max_calls=`, `?force=`) — fills `variation_group` from wger by `wger_id` (UPDATE-only, fill-if-null), same bulk-fetch shape as the content seed. **Not yet run** (needs the migration first). Returns coverage counts (`matched_to_wger`, `variation_groups_written`, `wger_had_no_group`, …).
...
- **Pending the two manual data steps** (run migration → run seed): until then the endpoint's variations query no-ops (missing column → non-fatal → `variations:null`), so the section simply doesn't render — the rest of the detail view is unaffected. Once seeded, verify a known wger variation group (e.g. Lunges ↔ Reverse Lunges) renders + taps through.
...
**Backlog (ROADMAP §9):** prompt/generation-size management — latency is dominated by the 2200-token output, and the 6KB input cap increasingly truncates real context as logged history grows. Levers: reduce output size; smarter context selection over blunt truncation. Not urgent now that SSE removed the timeout. **Not done this session:** model migration (Sonnet 5) was explicitly out of scope — a separate deliberate decision, not a timeout fix.
