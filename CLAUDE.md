...
- **Two sections in the panel:**
  1. **Past workout history** — `renderLogPastHistory()`, a deeper scrollable list (last ~20, `max-height:280px` scroll) built from the existing `workoutLog` global (**no new query** — same data the History tab and streak use). Each row's **🔁 Re-log** reuses the existing `reLogWorkout(id)` (fetches `/api/workouts/:id/full`, rebuilds notes from exercises + original notes, prefills the modal).
  2. **Saved templates** — the reused `renderRecentAndTemplates()` in a **new templates-only mode** (see below), each **▶ Use** reusing the existing `useTemplate(id)`.
- **Both taps prefill the Log Workout modal and never log immediately** — they call the pre-existing `reLogWorkout`/`useTemplate`, which open `#log-modal` prefilled for the athlete to review/edit and save through the normal flow. `use_count` behavior is unchanged: `useTemplate` bumps it (as the ▶ Use flow always did); re-log doesn't. No new API calls.
...
- **CSS is id-scoped** (`#log-past-card`/`#log-past-panel`/`#log-past-btn`/`#log-past-history-list`), no global class changes. A scoped `#log-past-panel #recent-workouts-card` rule strips that element's normal full-card chrome (bg/border/padding) so it reads as a plain nested section.
