...
## PT Brain — Session A (2026-07-25, Session 15): keystone join + Layer 1 + capacity + negotiation

Retires the fixed **3 near_term + 2 horizon** skeleton and the integer **4–6 `duration_weeks`**
clamp **for per-goal roadmaps only**. Phase count and each phase's week budget are now
**derived in code** from an honest per-goal timeline estimate and handed to the model as fixed
slots to fill. **Macro-roadmap paths (`MACRO_ROADMAP_SYS`, `adaptMacroRoadmap`) are untouched
and keep 3+2 until Layer 3.** All storage is additive jsonb — **no DDL**, no npm deps, no v2
files touched. Design of record: `ROADMAP.md` §7 → "NEXT DIRECTION — the 'PT Brain'".

### The governing split
**Code owns every number the model would otherwise be judged on.** The model authors names,
weekly targets, emphasis, completion signals, milestones and the plain-language *basis*; code
owns phase count, each `duration_weeks`, each horizon `estimated_range`, the capacity
arithmetic, and the dial lock. This is the direct lesson of the rejected 0.70 work-floor
(ROADMAP §6): a model must not optimize its own metric.

### Pre-work — profile 4 flipped to v1
`PATCH /api/profiles/4 {"profile_data":{"engine_v2":false}}` (set `false`, not deleted — the
top-level merge cannot delete a key, and both gates check `=== true`). Verified: `GET
/api/v2/today/4` → `engine_v2:false`; plan-ready `engine_v2_flag:false`; **`POST
/api/v2/cron/nightly` → `processed: 0`** (was selecting profile 4); `profile_data` intact at 24
keys with all 8 tiers, `schedule_v3`, `defaults` and 3 roadmaps preserved. The v2 residue
(`training_blocks` block 10, 7 `planned_sessions`, a fresh `v2_daily_cache`) is still in place
and now unreachable. Reversible with the same PATCH set to `true`.

### KEYSTONE JOIN — write side only
Optional `goal_ids: [<goal uuid>, …]` on `frequency_targets[i]`, `anchors[day][i]` and
`addons[i]`. **Nothing reads it — Layer 2 is the first consumer.** `buildScheduleInstruction`,
`buildWeekSkeleton`, `schedTargetDoneCount` and `schedAnchorStatus` are all untouched.

- **Editor**: `schedGoalPickerHtml(kind, idx, item, day)` renders a multi-select pill row on
  each target/addon row and (once an anchor exists) in the day editor; `schedToggleGoalLink`
  writes, and **deletes the key entirely when the last link is removed** so absent ≠ `[]`.
- **Round-trip is safe by construction, re-verified**: `loadSchedule()` rebuilds the schedule
  *container* but assigns the arrays/object **by reference**, so item objects keep unknown keys;
  `cleanProfileData()` is fully recursive and type-preserving. Precedent: `stackable`.
- **⚠ `schedSaveAnchor` was a destroy-site nobody had listed.** It did
  `currentSchedule.anchors[day] = [{activity, duration}]` — a fresh literal on **every** "Set
  Anchor" tap, wiping every other key on the anchor. That is the *normal* editing path, far more
  likely to fire than the AI builder. Now `Object.assign({}, prevAnchor || {}, {...})`, which
  preserves **any** unknown key, not just `goal_ids`. (Still collapses a migrated
  multi-activity day to one anchor — pre-existing, out of scope.)
- **Build-with-AI guard (G3)**: `schedStartAIBuild()` counts existing links
  (`schedCountGoalLinks`) and `confirm()`s when any exist; it snapshots `schedAIPrevSchedule`,
  and after the rebuild `schedCarryGoalLinks(prev, next)` re-attaches links **matched by
  normalised activity name** (ids are regenerated by the builder, so they cannot be the key).
  Adding `goal_ids` to the builder's own AI skeleton was **deliberately deferred** — it is a
  Haiku call whose output would need id validation of its own (ROADMAP §7).

### LAYER 1 — the phase-plan derivation
New goal fields (`profile_data.goals[i]`): `goal_type` ∈ `rehab | strength_load | endurance |
skill | habit | body_comp`, and `demand { sessions_per_week, minutes_per_session, hard,
min_viable_sessions_per_week }`. New: `goal.estimate` and `goal.roadmap.estimate` (see the
two-field shape in ROADMAP §7).

`derivePhasePlan(estimate)` (`server.js`):
```
W        = round((total_weeks_low + total_weeks_high) / 2)
nearSpan = min(W, NEAR_TERM_MAX_WEEKS = 16)
nearCount= clamp(round(nearSpan / 5), 1, 4);  if (nearSpan >= 5 && nearCount < 2) nearCount = 2
budgets  = nearSpan distributed as integers, remainder FRONT-loaded
horizon  = (W - nearSpan) split into clamp(round(rem / 12), 1, 3) blocks, each rendered
           "months X–Y" by formatMonthRange()
```
Every budget lands in **[2, 6]** by construction, and `nearSpan + Σhorizon == W`, **swept and
asserted for every integer 1..200**. The `nearSpan >= 5 → at least 2 phases` floor is what turns
a 6-week rehab goal into `[3,3]` rather than one undifferentiated block.

`renderPhasePlanForPrompt()` emits the two code-authored blocks the generator obeys (TIMELINE
ESTIMATE + PHASE PLAN). `applyPhasePlanToPhases()` then **enforces the plan on what came back**:
extra phases are dropped, `duration_weeks` and code-authored `estimated_range` are corrected,
and if the model returned *fewer* near-term phases the budgets are **redistributed across the
phases it did return** so the span still sums to the derived total — never padded with a
fabricated empty phase.

`resolvePhasePlanForGoal(goal)` handles the legacy case: a goal with no `estimate` keeps its
**existing** phase counts and durations (**deliberately unclamped** — profile 1's "Strength &
Integration" is 8 weeks and regenerating must never silently shrink it), falling back to 3+2
with model-authored `estimated_range` when there is no roadmap either.

### The aggressiveness dial — server-authoritative
`assumed_frequency` is the sessions-per-week stepper. `DIAL_LOCKED_TYPES = ["rehab"]` is a
**code** constant, not a prompt rule. `plan-setup` persists its proposal so `/estimate` has a
stored value to enforce against: for a locked `goal_type` an incoming `sessions_per_week` is
**discarded** in favour of the stored one and `dial_override_applied:true` is returned. The
client mirrors the lock for UX (`GRV_DIAL_LOCKED`) but is never trusted. Locked UI shows the
honest reason; `skill` shows a diminishing-returns note instead.

### CAPACITY + the fit check
`profile_data.capacity = { days_per_week, minutes_per_day, hard_sessions_per_week,
protected_days[] }` — global, on the profile, captured **inline at first goal intake**, edited
afterwards on the Profile-tab `#capacity-card` (hidden until capacity exists).

`computeCapacityFit()` is **two axes only**: minutes and recoverable hard sessions. **Session
count vs `days_per_week` is deliberately NOT an axis** — v1 routinely stacks several goals into
one session, so "6 sessions across 4 days" is normal, not a conflict. Goals with status `DONE`
or `PAUSED` are excluded (`CAPACITY_EXCLUDED_STATUSES`); goals with no `demand` are excluded and
reported as `no_demand`. No capacity captured → `has_capacity:false, fits:true` (cannot judge,
so never block).

### INTAKE NEGOTIATION — bounded to three levers
Slots into the real seam the client already had (two sequential calls in
`generatePersonalizedRoadmap`). Code computes the fit; the model writes the framing only. The
response is validated against **exactly** `slower | capacity | sequence`, in that order, with a
**code-authored fallback** if the model returns anything else (`model_levers_valid:false`).

Bounded writes, all via a re-post to `/estimate` — **no separate write path, and no coexistence
verdict is stored (that is Layer 3)**: `slower` lowers `sessions_per_week`; `capacity` returns to
Step 3 so the athlete edits the real numbers (never the model's prose); `sequence` sets
`sessions_per_week = min_viable_sessions_per_week` and appends an honest note to `estimate.basis`.

### Routes (three short Sonnet calls, deliberately separate requests)
`POST .../goals/:goalId/plan-setup` → proposes `goal_type` + `demand`.
`POST .../goals/:goalId/estimate` → persists the confirmed shape + capacity, runs the estimate,
returns the derived `phase_plan` and the **code** fit check. Also the single entry point for
every negotiation lever and for a later dial change.
`POST .../goals/:goalId/negotiate` → the three levers.
`GET .../capacity` → read-only snapshot for the Profile card.
**Not folded into `POST /roadmap`**: that is already a non-streamed 2500-token Sonnet generation
near Render's ~25s ceiling, and chaining another model call inside it would push it over.
`CALL_TYPE_MODEL` gains `goal_plan_setup`, `goal_timeline_estimate`, `goal_negotiate` → Sonnet.
**`goal_timeline_estimate`, NOT `goal_estimate`** — that key was retired as dead in session #31
(ROADMAP §9) and must not be resurrected.

### `enforcePhaseShape` — deterministic, count-only
Run after every adapt, scoped **exactly** to the near_term/horizon COUNT. Content edits (names,
targets, emphasis, signals, dates, status) pass freely. A reshape **throws**, which leaves the
stored roadmap untouched — and because `last_adapted_at` only advances on success, the weekly
trigger simply retries on the next workout save. Precedent: `enforceSingleCurrentPhase`.

### Code-level defenses
- **`ensureGoalDefaults()`** — a **shape normaliser, not a value inventor**. It nulls an
  out-of-enum `goal_type`, coerces a present `demand` to numbers/booleans, and derives
  `min_viable_sessions_per_week` from `sessions_per_week` when absent. It **never fabricates a
  `demand` for an unspecced goal** — writing defaults onto all 16 bare goals across profiles 1
  and 4 would have blown the capacity sum with numbers nobody entered. Wired beside
  `ensureGoalIds` at `GET /api/profiles/:id`, `PATCH /api/profiles/:id`, **and `POST
  /api/profiles`** (A4 — creation previously ran neither).
- **G1 `mergeGoalsPreservingProtected()`** — the Profile Builder's "goals" section hands the
  entire `profile_data` to a model and stores what comes back, and `PATCH` replaces `goals[]`
  wholesale (it merges only at the top level). The guard matches AI goals to stored ones **by
  id, falling back to normalised title**; **stored wins on every protected field**
  (`id, roadmap, estimate, goal_type, demand, intake_*, last_adapted_at, target_date, tier,
  priority, arc_state`); a stored goal the AI omitted is **kept, never deleted**; the AI cannot
  *invent* a protected field either. **Stored order is preserved** — array order IS goal
  priority (driving the rec prompt's 40/25/15% weighting), so letting the AI reorder was a
  silent re-ranking bug; deliberate reordering stays in the Prioritize UI. The real write site
  is **`pbFinishSection` (`public/index.html`)**, not `pbSaveCurrent`; `pbSaveReviewSection`
  funnels into the same function, so one guard covers both entry paths.

### Two bugs found LIVE during verification (not by inspection)
1. **`adaptGoalRoadmap` dropped `roadmap.estimate`.** It rebuilds the roadmap object from
   scratch and had no `estimate` key, so the first check-in after generation wiped the record of
   what the roadmap was built from and the "your settings changed" staleness banner could never
   fire again. An adapt reshapes phase *content* and never re-estimates, so `prev.estimate` is
   carried forward.
2. **`adaptGoalRoadmap` truncated a 5-phase roadmap at `max_tokens: 2000`** — reproduced **3/3
   at the same byte offset** on a real 3+2 roadmap; the JSON was cut mid-array and `parseAIJson`
   threw. **That is the legacy shape, i.e. profile 1's three roadmaps**, and the weekly
   auto-adapt is fire-and-forget with `console.error` only, so this would have been an invisible
   systematic failure. Raised to 3000 (generation of the same structure already used 2500; adapt
   reproduces more per phase).

### Verified live (profile 4, deployed build)
| Check | Result |
|---|---|
| (a) rehab wrist, estimate 4–10 wk | **2 near `[4,3]`, 0 horizon**; sum 7 == derived total; dial locked |
| (a) dial lock, server-side | client sent `sessions_per_week:7` → stored **5**, `dial_override_applied:true` |
| (b) 135→175 bench, estimate 16–32 wk | **3 near `[6,5,5]` + 1 horizon "months 4–6"**; sum 16 |
| (b) dial 2→3 | re-estimate **16–32 → 16–28** (honest diminishing returns, not linear) |
| (b) divergence | `goal.estimate` `16–28 @3x` vs `roadmap.estimate` `16–32 @2x` → stale banner true |
| (b) regenerate | version **4→5**, `adaptation_log` **3→4 preserved**, estimate caught up |
| (b) adapt on a NEW 3+1 shape | shape preserved across v2→v3 and v5→v6; estimate survives post-fix |
| (c) marathon | fit **420/300 min, 7/3 hard** → negotiation fired, `model_levers_valid:true` |
| (c) levers | exactly 3, order `slower,capacity,sequence` exact |
| (c) `sequence` write | `sessions_per_week` == `min_viable`, honest note in `basis`, **no `coexistence` key on the goal or on `roadmap_data`** |
| fits-cleanly path | (a) and (b) both `fits:true` → Step 4 never rendered |
| PAUSED exclusion | a constructed PAUSED goal with a large demand excluded as `status:PAUSED` |
| `ensureGoalDefaults` | `goal_type:"not_a_real_type"` → `null`; string demand coerced; `min_viable` derived; **8 bare goals stayed bare** |
| legacy 3+2 adapt | shape preserved, v8→v9, durations `5,6,6` unchanged, `estimate` stays null |
| keystone round-trip | `goal_ids` on a target AND an anchor survive PATCH + `cleanProfileData` + re-read |
| G2 anchor edit | activity→"MMA Class (renamed)", duration→75, **both `goal_ids` preserved to the server** |
| G3 confirm | fired with the correct count (2); **dismiss aborted** the rebuild, accept snapshotted |
| G3 carry-forward | a rebuild that wiped links restored **2/2** (target 1 id, anchor 2 ids) |
| v1 UI post-flip | profile 4 Today renders the v1 manual check-in gate + `#ai-card`; `isV2Profile()` false, `#v2-today` absent, `#v2-week-card` `display:none` |
| **profile 1** | goals sha256 **`0901b047d1c95f50` before and after — byte-identical**; no `capacity`, no `goal_type`/`demand` written |

**34 local assertions** also pass against functions extracted from the real shipped source
(the `v2FoldedCards.test.js` discipline): `derivePhasePlan` worked targets + a 1..200 sweep,
`ensureGoalDefaults`, `computeCapacityFit`, `enforcePhaseShape` (both shapes, content-edit
pass, count-change throw), `mergeGoalsPreservingProtected`, `schedCarryGoalLinks`.

**Not verified live:** the Step 3 / Step 4 screens were exercised through their real endpoints,
not by clicking through the intake UI end-to-end in a browser; the full Build-with-AI rebuild
was exercised via `schedStartAIBuild` + a simulated rebuild rather than a real Haiku build.
...
## BUG 4 — `needs_regeneration` must be RECOMPUTED, never carried (2026-07-28, session #44)

`arc_state.needs_regeneration` could be **set but never cleared**. Counted across the whole repo:
**4 write sites, 5 reads, ZERO clears.** `computeArcState` seeded it from the previous arc
(`!!(prevArc && prevArc.needs_regeneration)`) and `applyTimelineFlex` wrote `true` in three places;
**no `= false` existed anywhere.** Once set, the flag survived every evaluation, every adapt and
every regenerate — permanently. The athlete's experience was a **banner → regenerate → banner loop
with no exit.**

**Session #42's `carryArcForward` did not cause this — it exposed it.** Pre-#42 a regenerate
destroyed `arc_state` wholesale, which accidentally dropped the flag until the next evaluation.

### Why a naive recompute would have been wrong — measured, not theorised

Chaining five real evaluations offline against profile 4's wrist goal (real shipped
`computeArcState` + `applyTimelineFlex`, mock Supabase, each PATCH written back so evaluations chain
as production would) exposed a **one-evaluation flicker window**:

| fire | drift | `flex_streak` | flex outcome | a naive recompute would read |
|---|---|---|---|---|
...
runs**. **Fire 2 is the wrist goal's very next evaluation**, so a naive fix would have broken on the
first goal tested.
...
### The fix (D1) — two edits, one logical change

1. `computeArcState` seeds `needs_regeneration: false` instead of carrying the previous value.
2. `applyTimelineFlex` hoists the absorbability math **above** the `flex_streak` gate, so **every
   call writes the flag**; the streak now gates only the phase-duration **mutation**.

```
if (!arc) return null;
arc.needs_regeneration = false;              // recomputed every evaluation
if (|drift| < 2) return null;                // nothing to absorb -> correctly false
...upcoming...
if (!upcoming.length) { arc.needs_regeneration = true; return {...}; }
...compute bounded / clamped...
if (clamped) arc.needs_regeneration = true;
if (flex_streak < ARC_FLEX_STREAK_REQUIRED) return null;   // MUTATION ONLY from here
...mutate...
```

**Approved widening:** `!upcoming.length` now fires below the streak gate too — zero upcoming phases
is unabsorbable regardless of how long the drift has been sustained. This is the only behavioural
change beyond the flag itself; flex thresholds, the mutation path and the return shapes are
otherwise byte-identical to pre-fix (asserted).

**⚠ STANDING RULE: the streak gate guards the MUTATION only, never the assessment.** If a future
change adds another early return to `applyTimelineFlex`, write the flag before it.

### D2 — no special handling on regenerate

`carryArcForward` still carries `arc_state` verbatim (including the flag) across a regenerate, and
that is correct: the phases just changed, so absorbability is genuinely **unknown** until reassessed.
Carrying `true` is the conservative direction; clearing optimistically would make the banner lie in
the **opposite** direction — and for the wrist goal the post-regenerate answer is still `true`.
Stale-true window is **≤24 h** (the app-open gate on `last_evaluated`) or until the next workout
save. **Considered and DECLINED:** calling `evaluateArcsForProfile` from the regenerate handler — it
would close the window but adds latency to an already-long Sonnet call, for a window that self-heals.

### D3 — the copy was lying, and that is why the athlete looped

The old CTA promised *"Regenerate it to match reality."* That button **cannot** deliver it: the
drift is **earned position vs calendar**, and rebuilding the phases resets the calendar while leaving
earned position untouched. Both readers now state the real numbers:

- **drill-down banner** (`grvEstimateHeaderHtml`) — *"This plan is behind where you actually are /
  You've earned 0 of 5 weeks on this goal, so the remaining phases can't stretch far enough to
  match. Rebuilding the phases won't move your earned position — only logged sessions do that."*
  CTA: **"Rebuild the phases anyway"**.
- **goal-card ⚠ flag** (`renderProfileGoals`) — same framing, one line.
- `grvArcWeeks(n)` trims a trailing `.0` so a fractional earned position reads cleanly.
...
### Regenerate in-flight guard

`grvRegenInFlight` — set **after** the missing-goal early return (so it can never strand), released
on a trailing `.then()` that runs on **both** the success and the `grvShowFullError` paths, so Retry
is never blocked. **Honest note on exposure:** `c.innerHTML = <spinner>` already removes the button
synchronously on the first click, so a plain double-click was largely self-protecting — session
#43's claim was overstated. The residual windows are a missing `#grv-content` (the `if (c)` skips),
the Retry path, and any future refactor that renders the banner where the spinner does not overwrite.
The guard makes "one at a time" an explicit invariant instead of an emergent side effect.

### Verified

Real shipped functions, **both file versions**, over-capture guard + mandatory re-parse.
(a) 5-fire chain replayed post-fix — fire 2 now TRUE, `arc_origin 2026-06-29` every fire, flex
mutation shapes identical; (b) the CLEAR proven at unit level **and end-to-end through the real
route** (seeded `true` → recomputed `false` via a clean unclamped flex, then correctly re-set once
the room ran out — the two-way behaviour working); (c) all three true-branches; (d) the wrist goal's
real shape still TRUE; (e) `!upcoming.length` at streak 0 and 1; (f) **profile 1 legacy byte-identical
pre vs post** — it has 0 arc-eligible goals so `evaluateArcsForProfile` returns at the
`!arcGoals.length` guard before `computeArcState` is reached; (g) suites **213 → 249**.

**Verified live** (profile 4, admin sweep — both directions in one call): wrist `needs_regeneration:
true` with flex `lengthened [5]→[6] CLAMPED` (honest — drift −5 genuinely unabsorbable); bench
`needs_regeneration: FALSE` with flex `[5,5]→[6,6]`, `clamped: false`. Both `arc_origin`s intact;
`roadmap.version` unchanged on both, so no AI ran. Immediate re-fire of the user-facing route →
`{skipped:true, reason:"fresh"}`.

## Sandbox seeder (2026-07-28, session #44)

`POST /api/debug/seed-sandbox-workouts/:profileId` + `POST /api/debug/purge-sandbox-workouts/:profileId`,
both ADMIN_SECRET-gated and **dry-run by default** (`?apply=1` to write). Full workflow doc:
[`SANDBOX.md`](SANDBOX.md).

**⚠ IT CLOSES NO LEDGER ROW.** Rows 11, 13, 19, 20 and 28 in `ROADMAP.md` §7 are **real-data-only**
by definition — they wait on genuine athlete history or a genuine model response. Seeded history is
a development aid for *exploring* behaviour, never evidence a shipped behaviour works.

- **Profiles 1 and 4 are denylisted unconditionally** (403, hard-coded — confirmed live on both).
  1 is the athlete's real profile; 4 is the PT Brain test bed whose arc state backs open ledger rows.
- **Writes go DIRECT to Supabase, never through `POST /api/workouts`** — that route fires
  `extract-exercises` (Haiku per save), `evaluateArcsForProfile` and `maybeAdaptAllRoadmaps`
  (Sonnet). Seeding a year through it would cost real money and bury the arc under hundreds of
  adapts. **Zero AI calls during seeding.**
- **Every row carries the `[SEED]` notes marker**; the purge deletes only marked rows for that
  profile, so a sandbox is fully resettable and an athlete-logged row always survives.
- **Input is a generator spec, not hand-written rows:** `{start_date, weeks, pattern[], gap_windows[]}`.
  **The explicit gap windows are the point** — they are how decay and re-ramp get driven deliberately.
- **Never seeds the future** (clamped to the athlete's local today) and caps a plan at 2000 sessions.
- **`ts` is derived from the DATE, not `Date.now()`** — `GET /api/workouts` orders by `ts.desc`, so
  wall-clock `ts` would scramble seeded history into insertion order.
- Workout ids are correlated back **by `ts`**, not by trusting PostgREST to echo insertion order.
- `?evaluate=1` runs one arc evaluation after seeding (pure code, zero AI).

**The load-bearing verification is not "did rows get written" but "do these rows QUALIFY under the
real arc matcher"** — a seeded history whose exercise names don't map to muscle groups produces zero
qualifying sessions and teaches nothing. So the tests extract the **real shipped `makeTargetMatcher`
/ `muscleGroupsForExercise`** and run them against seeded fixtures: every seeded strength session
satisfies an upper-body target, a **single**-exercise session correctly does **not** (so the pass is
on merit, not accident), grip-only names never carry a session, a `done:false` session never
qualifies, and a declared gap produces a real ≥21-day hole in the date series.
...
## Profile-1 diagnostic (2026-08-02, session #46) — how the daily rec really gets built

**Report-only session, zero writes.** Recorded here because three of the findings are permanent
facts about how this app assembles a recommendation, not one-off bugs.

### The daily-rec category is NOT model-chosen — Option 1 is picked in code

`buildScheduleInstruction()` (`public/index.html:10265`) decides Option 1's activity before the
model sees anything:

1. **today's anchor** (`schedule.anchors[todayKey]`) → Option 1 is locked to it verbatim; or
2. **the `frequency_target` with the largest raw deficit** (`times_per_week - done`), `+0.5` only
   when today is its `suggested_day`, ties broken by **array order** via a strict `>`; or
3. no outstanding target → "any category that best fits readiness and micro-goals".

Options 2 and 3 are model-chosen but written under that instruction plus the `VARIETY RULE` and the
`SKIP RULE`, both of which defer to it ("the schedule instruction always wins").

**Three properties of that scorer that matter and are easy to miss:**

- **`times_per_week` is the whole ranking.** A 2×/week target structurally outranks a 1×/week target
  until it is at least half-met. There is no goal-priority input at all, even though the keystone
  join (`frequency_targets[i].goal_ids`) exists.
- **`schedTargetDoneCount()` counts by INFERRED CATEGORY, not by target identity.** It maps the
  target's `activity` string through `inferWorkoutCategory()` and counts every done workout this week
  whose `type` maps to the same category. So **two targets that infer to the same category share
  their done-count** — profile 1's "Upper Body Strength" and "Lower Body Stength" both → `strength`,
  and one strength session clears both.
- **`inferWorkoutCategory()` is first-match-wins in a fixed order** (`rest → martial_arts → strength
  → cardio → mind_body → rehab → sports`). The AI-generated `workouts.type` title routinely names
  several categories, so *"Cardio (Stationary Bike, 20min) + Strength (Upper Body) + Mind & Body"*
  counts as **strength only** and advances neither the cardio nor the yoga target.

Measured consequence on profile 1 over the real last 14 days (simulated with the extracted shipped
function against the real log): **Yoga ordered into Option 1 on 7 days, MMA anchor on 4, Cardio on 1,
Strength on 2** — and the 2 were 2026-08-01/02, the first days Cardio (3/2) and Yoga (2/2) were both
satisfied. Today's real cached rec is consequently **all three options strength**.

### ⚠ `libExercises` is empty on a cold boot, and the rec prompt depends on it

`bootApp()` **never calls `loadLibrary()`** — the only call sites are `showTab('library')` and the
Library-tab renderers. But `libExercises` is the sole input to **both** rec-prompt builders that
carry exercise-level detail:

- `buildLog(days)` → the `RECENT N-DAY LOG` block
- `buildExerciseHistory(limit)` → the `RECENT EXERCISE HISTORY` block, i.e. **every PERSONAL BEST**

Measured in a real browser (cold boot, Today tab, 9 s settle): `libExercises = 0`, `workoutLog = 60`.
`buildExerciseHistory` early-returns `''`; `buildLog` returns the literal string
**"No exercises logged in the last 7 days."** — which is false, and which sits directly below a
`THIS WEEK SO FAR` block reading *"Sessions completed: 7"*. **4,274 chars of strength/progression
signal absent from the rec the athlete actually receives.**

The two **briefs** are the accidental backstop: they are generated **server-side from all-time
history** and are in the prompt in both states, which is why a cold-boot rec can still cite
"22.5 lbs" (`historical_brief`) and "last logged strength session was June 22" (`coaching_brief`).

### `fetchAI({auditOnly:true})` — the measurement, and how to run it write-free

`auditOnly` returns the assembled `{sections, systemPrompt, userMessage, durations, intensity}`
**before** any model call and writes nothing (`public/index.html:4855`). Session #46 ran it against
production profile 1 from a real browser under a **network-layer non-GET block** (Playwright
`context.route`, every non-GET fulfilled locally and never sent). Five POSTs were intercepted:
`evaluate-arcs`, `week-preview`, `goal-progress` ×2, `roadmap-data`.

**⚠ Two GET endpoints WRITE as a side effect and must be stubbed for a genuinely zero-write run:**
`GET /api/profiles/:id/daily` (fire-and-forget upserts `daily_steps` + `daily_sleep`, stamps
`last_synced_at`) and `GET /api/profiles/:id/micro-goals` (PATCHes `micro_goals.current_value`
whenever the recompute differs). Stubbing them means `biometrics` renders with `?` placeholders and
`microGoals` reads 0 — state that, don't gloss it.

Measured result, profile 1, 2026-08-02:

| | cold boot | after `loadLibrary()` |
|---|---|---|
| `exerciseHistory` | **0** | 3,548 |
| `recentLog` | **39** | 765 |
| `systemPrompt` | 6,934 | 6,934 |
| `coachingBrief` | 2,365 | 2,365 |
| `timeBudget` | 2,180 | 2,180 |
| `historicalBrief` | 2,157 | 2,157 |
| `focusOverride` | 1,861 | 1,861 |
| `roadmapEmphasis` | 1,572 | 1,572 |
| `arcState` | **0** | **0** |
| `_total / _budget` | 22,779 / 28,000 | 27,053 / 28,000 |
| `_trims` | **none** | **none** |

**The trim ladder does not fire.** `timeBudget` 2,180 = session #39's 1,539 + Layer 4's +641 depth
block, which cross-checks exactly. `arcState` 0 confirms profile 1 has zero arc goals.

### The manual category pill is write-free — re-confirmed in source

`fetchAI`'s `altOpts` branch (`:4972`) assigns `altRec`, calls `renderAI()` and **returns before**
the `localStorage.ac_cache` write (`:4991`) and before `cacheAIRecOnServer()` (`:4993`). The only
...
## History-Informed Intake (2026-08-04, session #48) — question shaping

**Before intake questions are generated, code aggregates the athlete's FULL logged history and
hands the model a small computed rollup.** The model uses it to ask better questions and nothing
else. `server.js` only; **no migration, no client change, no new dependency.** Shipped `3ca4ba8`.

### The defect, located precisely

The new-goal pipeline is five sequential calls, four of them model calls:

| # | Step | Model | What it received BEFORE this session |
|---|---|---|---|
| 1 | `GET .../goals/:goalId/intake` | Haiku | title, `goal.type`, description, `ai_prompt_context` **800 chars** |
| 2 | `POST .../intake` | — | the athlete's typed answers |
| 3 | `POST .../plan-setup` → `demand` | Sonnet | title, description, answers, `ai_prompt_context` **600 chars** |
| 4 | `POST .../estimate` → `estimate` | Sonnet | the above + `goal_type` + the step-3 demand, ctx **500 chars** |
| 5 | `POST .../roadmap` | Sonnet | **the first and only step that read a logged row** |

`estimate.total_weeks_low/high` is what `derivePhasePlan()` converts into phase count and each
phase's week budget. **So `demand` and `estimate` were 100% self-report, and history arrived one
step too late** — by the time step 5 saw the log, the timeline and the phase skeleton were fixed.
Two athletes with identical goal text and opposite logs got materially the same roadmap; only the
prose differed. That is exactly the defect the athlete described.

### `getTrainingHistorySummary(profileId, {keywords, today})`

Two outputs, deliberately — the same split Layer 2 uses for `arc_state`:
- a **structured object** (~19 KB on a real profile) for CODE. It never leaves the server.
- a **rendered text block** for the MODEL. **Measured 202–1,120 chars.** The model never sees a row.

`buildTrainingHistorySummary(workouts, exercises, todayYmd, opts)` is **pure over its inputs**, so
it is directly unit-testable against a fixture; the fetch is the caller's job.

Profile-level: `total_sessions`, `distinct_days`, first/last date, `span_days`,
`sessions_per_week_{4w,8w,12w,all_time}`, `longest_gap_days` + from/to, `current_gap_days`,
`category_totals`, `per_month`. Per-exercise: `first_date`, `last_date`, `sessions`, and all four
best fields. Goal-relevant slice: matched exercises **with their own first/last/sessions/best**,
matched session types, `by_source` coverage, `confidence`, `specific_terms`, `specific_term_hits`.
Endurance view: distance rollups, **`null` when there is no distance data at all** — which is
profile 1's real state (0 of 332 rows carry `distance_miles`).

**Self-capping like `buildArcStateContext`:** `HX_MAX_EXERCISE_LINES = 6`, `HX_MAX_MONTHS = 6`,
`HX_BLOCK_CHAR_CAP = 1600`, and the cap drops **whole lines** — a half-printed week number is worse
than an omitted line.

### THE LAYERED MATCHER — exercise-level FIRST (the athlete's steer)

| layer | source | sets |
|---|---|---|
| **PRIMARY** | per-day `exercises` rows — name plus weight/reps/duration/distance | `confidence: "exercise"` |
| **SECONDARY** | workout **title + notes** via an ALL-CATEGORIES parse | `confidence: "category"` |
| **NONE** | neither fired | `confidence: "none"` |

**The secondary layer is MANDATORY, not optional, and the audit's own numbers are why:** cardio
sessions frequently log **zero** exercise rows (3 of 10 sampled on profile 1) and `distance_miles`
is NULL on **all 332** of profile 1's rows. An exercise-only matcher would report "no evidence" for
real cardio work. **`by_source` reports how often each layer actually fires** — on profile 1 a
running goal is `{exercise: 0, category: 25, category_only: 25}`; on the seeded runner fixture it is
`{exercise: 20, category: 61, both: 20, category_only: 41}`.

**Tokenization, on both layers.** Word-boundary matching (whole normalized tokens, never
`indexOf`), **≥4 chars unless whitelisted** (`HX_SHORT_TOKEN_OK`: run, row, jog, ski, gym, abs, hip,
arm, leg, 5k, 10k, ohp, rdl), bare numbers dropped, and **hyphen-normalized on BOTH sides**:
`hxNameForms()` exposes each word plus the whole name joined, and `hxGoalTokens()` emits the joined
compound for hyphenated/spaced pairs. Goal `"pull-ups"` → `pullup`; stored `"Pull-Up"` → `pullup`.

**`HX_GENERIC_TOKENS` — a match made ONLY of generic tokens is not exercise evidence.** It falls
through to the category layer instead. This is what stops a "Bench Press" goal claiming "Overhead
Press" on the strength of `press` alone.

**`hxAllCategories()` is a NEW function; `inferWorkoutCategoryServer` is untouched.** The ordered
first-match-wins version is still used everywhere else and is asserted byte-identical by test.
67% of profile 1's real workout titles name 2+ categories, and first-match-wins undercounts its
cardio by **15 of 25 sessions**.

### Measured against profile 1's REAL log — the four acceptance criteria

| # | criterion | result |
|---|---|---|
| AC1 | crunch false positive **4 → 0** | ✅ `run` was a substring of "C-**run**-ches"; the old rule matched `Crunches ×3` + `Standing Oblique Crunch ×1` and **zero** real runs |
| AC2 | cardio recovery **15 of 25** | ✅ first-match-wins 10 → all-categories 25 |
| AC3 | `Pull-Up` visible to a pull-up goal | ✅ matched at `confidence: exercise` |
| AC4 | the honest negative | ✅ *"NONE of those sessions mentions run or half or mile specifically. There is NO direct evidence of this exact activity."* — the block never presents crunches as running |

### Two defects in this code found BY the measurement, not by inspection

1. **`pull` alone dragged "Band Pull-Apart" into a pull-up goal.** `pull` / `push` / `leg` / `arm` /
   `core` / `upper` / `lower` are movement-FAMILY words, not exercise identities — now generic. The
   compound (`pullup`) carries the real match. Re-measured: `["Band Pull-Apart","Pull-Up"]` → `["Pull-Up"]`.
2. **A 28-minute run rendered as `28:00 hold`.** That is the `duration_minutes` overload (ROADMAP
   §6 — hold duration AND session length, still undistinguished in v1). Disambiguated by category
   for the **LABEL ONLY** via `HX_SESSION_LENGTH_CATEGORIES` (cardio / martial_arts / sports /
   mind_body = session), the same rule Engine v2 used. No stored value is touched.

Compound generation also leaked junk tokens (`225lbs`, `1mile`, `20pull`) into the rendered
sentence; compounds built from a bare number are now skipped, and the "NONE of those sessions
mentions…" line names at most **3** terms.

### Honest limits — stated, not glossed

- **`press` → "Overhead Press" survives token overlap.** The generic-token rule stops it being
  reported as *specific* evidence, but overlap on a shared word is a structural limit of token
  matching. The mitigation is that an uncertain match produces a **question**, never a silent
  estimate adjustment.
- **Cardio's missing exercise rows** mean the category layer is doing the work for endurance goals,
  and it is labelled *"matched only at the activity-category level, NOT on a named exercise —
  weaker evidence"* in the block itself.
- **`distance_miles` is NULL across all 332 of profile 1's rows**, so #46 A2's proposed
  "recent distances/paces" view has nothing to compute from on the real athlete. `endurance` is
  `null` rather than a fabricated zero.
- **`makeTargetMatcher` cannot run off a goal title** and was NOT used: `activityMuscles()` returns
  `[]` for 6 of 7 real goal titles, and `satisfies` returns false immediately on empty muscles.

### Wiring — `GET /intake` ONLY

`plan-setup` and `/estimate` receive **nothing** this session; estimate grounding (b) is designed
and documented but **not built**, pending explicit approval. Asserted by test.

The system prompt gains a conditional instruction block: don't ask what the log already answers; do
ask about what it contradicts or leaves ambiguous; you may reference a fact so the athlete sees you
read their log; **never restate a number that is not in the block and never contradict one**; and
when there is no history, do not state or imply any logged fact. A permanent one-line diagnostic
logs `has_history / days / matched / confidence / by_source / specific_hits / block_chars` per call.

### Cold start and failure

- `has_history: false` → a **202-char** honest statement (*"none on record… Do NOT state or imply
  any logged fact"*). Verified live on profile 9 (0 workouts, 0 exercise rows).
- **A failed read returns `null`, and a null summary emits NO block at all.** This is the session
  #47 Phase 1 bug class: an *unloaded* history must never be rendered as *"no exercises logged"*.
- History exists but nothing matches → *"NO logged sessions match"*, while still reporting the
  profile-level history. It never claims zero history.

### Blast radius — proven, not asserted

`public/index.html` is **byte-identical** to the pinned pre-change commit `dc72f04`, so the
daily-rec prompt has a **0-char delta by construction** — the rec is assembled in the browser and
this session did not touch it. `extractGoalKeywords` / `GOAL_STOP_WORDS` / `ARC_WEAK_KEYWORDS` /
`getGoalSessionDates` / `inferWorkoutCategoryServer` are all byte-identical to the same commit.
Profile 1's three migrated goals still evaluate at `tier 2` / `keyword` / `qualifying_sessions_28d
0` / `arc_origin 2026-08-03`, unchanged. **Profile 1 `profile_data` sha256
`4313ea6a3896b172b5a2e387627ec6bc1075007f416ba460027c803e8130b9d6` identical at session start and
session end.** Profile 4 sha `c356df4b8457d59b`, untouched.

### The conflict rule — DESIGNED AND APPROVED, deliberately NOT built this session

Code detects → gates on **materiality AND match confidence** → the model **asks** → code uses the
reconciled answer → unresolved defaults to the **conservative (longer)** reading, stated in
`estimate.basis`. Two binding guards: **never raise a discrepancy on `confidence: "none"`** (a
low-confidence miss means "the log has no opinion", not "the athlete is wrong"), and **nothing
writes to the goal from a discrepancy** — it only changes which questions get asked. The
`specific_term_hits` field is the signal a discrepancy check would gate on; it is already computed
and exposed. Building it belongs with (b).

### (b) ESTIMATE GROUNDING — approved as DESIGN, NOT built. Exact insertion points.

| route | insertion | measured cost |
|---|---|---|
| `POST .../plan-setup` | block appended to the user message, before "Propose how this athlete should train" | 1,460 → ~2,500 chars |
| `POST .../estimate` | block appended, plus a **code-derived starting-point classification** | 1,412 → ~2,452 chars |

**The load-bearing rule:** the model still authors the week range, but the *starting-point
classification* it reasons from is **code-derived from the aggregate, not inferred by the model** —
a small band (e.g. `trained_recently` / `detrained` / `no_relevant_history`) computed from
`per_week_8w`, `current_gap_days`, `longest_gap_days` and `goal_relevant.sessions`, stated in the
block, with the prompt requiring the range to be consistent with it. **No new model-authored number
reaches storage** — `derivePhasePlan` still owns phase count and week budgets, `normalizeDemand`
still clamps, and the dial lock is still server-authoritative.

**Both Layer-1 prompts are server-side with their own `max_tokens` (500 / 400). They share nothing
with `fetchAI`'s 28,000-char daily-rec guard.** Adding the block costs that guard 0 chars.

### Test harness

`server/historyIntake.test.js` — **28 tests**, real shipped functions extracted from `server.js` by
source slicing and evaluated in a `vm` sandbox. The extractor is comment/string/template/
regex-literal aware, has an **over-capture guard that throws**, and **re-parses every slice**. The
"before" reference is the **pinned commit `dc72f04`, never `HEAD`** (session #44's learning), with
an assertion that the pinned commit does *not* already contain this session's work.

**The harness caught a real imprecision in its own assertions:** a "must not call
`extractGoalKeywords`" check failed because the new section's own doc comment *names* those
functions to say it does not use them. Fixed by checking `stripToCode()` output — comments are not
calls — plus a positive assertion so the guard cannot be vacuous. **Suites 330 → 358, all green.**
...
> DESIGNED, DOCUMENTED and NOT BUILT — it needs explicit approval. There is no open bug in what
> shipped.**
>
> | # | Work | Why |
> |---|---|---|
> | **1** | **(b) ESTIMATE GROUNDING — awaiting approval** | The design and its exact insertion points are in "History-Informed Intake" above. This is the half that actually changes the roadmap: `estimate` feeds `derivePhasePlan`, which owns phase count and week budgets. Until it ships, two athletes with identical goal text still get the same timeline — (a) only changes the questions. **Audit is already done; this is a build session if approved.** |
> | **2** | **Shimmy runs the malformed-date repair SQL, then the CHECK constraint** | Unchanged from #47. `migrations/2026-08-03_repair_malformed_workout_dates.sql` → verify (§A) → repair (§B) → confirm (§C, must return 0 rows), THEN `…_workouts_date_format_check.sql`. **One judgement call is flagged in the file header:** 7 of the 8 rows land on a day that already has a logged workout. |
> | **3** | **Shimmy links the three migrated goals to their schedule targets** | Unchanged and still the highest-value athlete action. All three sit at `tier 2` / `keyword` / **0 qualifying sessions** — re-confirmed live this session. **Until this is done the arc cannot move off position 0.** |
> | **4** | **First real weekly adapt on migrated profile 1 — ~2026-08-10** | Confirm `arc_origin` is still `2026-08-03` and `arc_state` still present on all three while `version` increments and each gains a `weekly` entry. |
> | **5** | **Arc Tier-2 matching improvement — NEWLY SCOPED by this session** | The arc's keyword tier has the exact defects session #48 measured and fixed elsewhere: substring matching (`run`→`Crunches`), hyphen asymmetry (`pullups` never reaches `Pull-Up`), and first-match-wins category inference. `hxGoalTokens`/`hxNameForms`/`hxAllCategories` are the proven replacement. **Deliberately NOT applied to the arc this session** — it would move arc evidence for every unlinked goal, which is all three of profile 1's. Its own scope, with its own before/after verification. |
> | **6** | **Row 28 / item (c) — WAIT, do not build.** | Unchanged. Route (1): profile 4 accumulates real qualifying sessions until a genuine peak-then-gap forms. **Do not synthesise a re-ramp.** |
>
> **Fixture profiles 10 (HX Runner) and 11 (HX Nonrunner) are the motivating-case bed** — identical
> seeded structure, differing by exactly 20 `Easy Run` sessions. Profile 9 stays the cold-start bed
> (0 workouts). Purge with `POST /api/debug/purge-sandbox-workouts/{10,11}?apply=1` when done.
>
> **Read before starting:** "History-Informed Intake" above (the layered matcher and its honest
> limits), ROADMAP §6 → "Session #48 findings", §7 ledger rows **51–54**, and §7 → "⚠ PER-GOAL vs
> MACRO" (still the easiest thing to get wrong).

---

## Superseded next-session block (session #47) — kept for the record
...
> measured and DISPROVED. Nothing was fixed — session #46 was report-only, zero writes.**
>
> **THE IMMEDIATE WORK:**
>
> | # | Work | Why |
> |---|---|---|
> | **1** | **FIX THE COLD-BOOT EXERCISE-HISTORY RACE** (§9, session #46) | **The highest-value fix on the board and the prerequisite for #4.** `bootApp()` never calls `loadLibrary()`, so `libExercises = 0` when the day's rec is generated: `exerciseHistory` **3,548 → 0 chars** and `recentLog` **765 → 39 chars**, the 39 being the literal *false* line "No exercises logged in the last 7 days." next to a block saying "Sessions completed: 7". **Recommended shape: make `fetchAI` await an exercise fetch the same way it already awaits `microGoalsReady`** — deterministic, and the pattern is already in the function. **Fix the honesty bug too:** an unloaded array must not be reported as a zero. This is the second instance of the race #43 fixed for `/goal-progress`; §9 predicted more existed |
> | **2** | **TRACE THE MALFORMED-`workouts.date` WRITE PATH — it is still producing rows** (§6, §9) | 8 rows now, not the 6 on record. **ids 100143 (`'10:14'`) and 100144 (`'10:52'`) were written 2026-07-29, three seconds apart.** These rows are invisible on the calendar, dropped by the arc replay's `Date.parse` filter, and count toward no streak or target. `workouts.date` is `text`, `POST /api/workouts` forwards `req.body` verbatim, and the future-date guard only engages when `body.date` parses. **Find the client path first; repairing the 8 existing rows is a separate review-then-apply decision** (their real dates are recoverable from `ts`, but `ts` is a last-write stamp) |
> | **3** | **Weekly-path verification — the BENCH goal on profile 4, on/after `2026-08-02T02:47Z`** (ledger row 33) | **Unchanged and now UNBLOCKED by the calendar** — that timestamp has passed. Still the last unverified link in BUG 3's fix. Save a workout on profile 4, then confirm the bench goal's **`roadmap.arc_origin` is still `2026-07-26`** and `arc_state` is still present, **while** `roadmap.version` incremented and `adaptation_log` gained a `weekly` entry. **⚠ Do NOT use the wrist goal** — its regenerate reset `last_adapted_at` to 2026-08-04 |
> | **4** | **Profile-1 migration — A1 IS ANSWERED, and the answer is "migrate, don't scope"** (§7 → A1) | **The format concern does not survive measurement.** The ONLY thing a migrated goal injects into the rec prompt is `buildArcStateContext()` (one block, cap 950); `capacity` and `coexistence` are Profile-tab-only; and rec FORMAT is governed by `buildResponseShapeSpec` / `buildTimeBudgetContext` / `REC_DEPTH_TIERS` / `resolveOptionDurations`, **none of which read goal shape**. **And the sandbox comparison was invalid: profile 9 has 0 workouts, 0 exercise rows, 0 briefs, 0 micro-goals and no cached rec at all.** Remaining cost is unchanged: regeneration overwrites three roadmaps carrying real `adaptation_log` history — use the Coach-Chat regen path, not the reset path. **Still an athlete decision. Gated on #1** |
> | **5** | **Row 28 / item (c) — WAIT, do not build.** | Route (1) by decision: profile 4 accumulates real qualifying sessions on the bench goal until a genuine peak-then-gap forms. No date will be asserted for the lost origin. **The sandbox does not count as data for this** |
>
> **What session #46 established (all measured, profile 1, zero writes):**
> - **The April data is NOT gone.** 92 workouts all-time (Apr 24 · May 32 · Jun 13 · Jul 12 · Aug 3
>   + 8 malformed), earliest 2026-04-07, April carrying 145 exercise rows across all 24 of its
>   workouts. **`loadWorkouts()`'s `limit=60` reaches back to exactly 2026-05-09**, so 32 real
>   sessions sit outside every `workoutLog`-driven surface. The Today-tab "Log past workout" panel is
>   the only place that can page past it.
> - **The weekly adapt IS firing** — `last_adapted_at` 2026-07-30 / 2026-07-27 / 2026-07-27 with real
>   `weekly` entries. The roadmap CARD structurally cannot show it: legacy roadmaps use time-elapsed
>   progress (~1.8 %/day), phase status is date-derived, and the new analysis lands only in a
>   collapsed `adaptation_log`.
> - **The rec prompt is NOT being trimmed** — `_trims: none` in both states.
> - **`inferWorkoutCategory` + a raw-deficit scorer are what pinned Option 1 to Yoga/Cardio** for 8
>   of the last 12 non-anchor days; it self-corrected 2026-08-01 and today's real rec is all-strength.
>
> **Standing facts recorded this session — do not re-derive them:**
> - **Option 1's category is chosen in CODE, not by the model** (`buildScheduleInstruction`).
> - **`GET /api/profiles/:id/daily` and `GET /api/profiles/:id/micro-goals` WRITE as a side effect.**
>   Any "read-only" probe must stub both.
> - **The arc replay already reads FULL history** (`arc_origin` → today, `limit=2000`/`8000`). The
>   adapt reads **10** workouts; goal-progress and the daily rec read **60**. At profile 1's real rate
>   (5.5 sessions/week) **60 rows ≈ 11 weeks**.
> - **Full-history ANALYSIS must never mean full-history ROWS in a prompt** — the fix shape is a
>   code-computed rollup (§7 → A2), not a bigger window.
>
> **Read before starting:** `ROADMAP.md` §6 → "Profile-1 diagnostic — findings", §7 → "A1 ANSWERED" /
> "A2 ANSWERED" / "A3 ANSWERED", §9 → the session-#46 block, and §7 → "⚠ PER-GOAL vs MACRO" (still
> the easiest thing to get wrong).
>
> **Carried forward, do NOT chase with synthetic data:** ledger rows 11, 13, 19, 20, 28, 41.
> **L1–L4 backlog stays parked. Engine v2 remains PAUSED and its strategy REJECTED going forward.**

## Superseded next-session block (session #45) — kept for the record

> **⟶ SUPERSEDED by session #46's block above. Item 1 (the sandbox re-runs, ledger row 41) is
> unchanged and still blocked on the athlete; item 2 (the weekly-path check) is carried forward and
> is now unblocked by the calendar. Original text follows.**
...
> open bug.** One shipped-unverified row is time-gated; one is data-gated.
>
> **THE IMMEDIATE WORK:**
>
> | # | Work | Why |
> |---|---|---|
> | **1** | **Weekly-path verification — the BENCH goal, on/after `2026-08-02T02:47Z`** (ledger row 33) | The **last unverified link in BUG 3's fix**. `carryArcForward` is now verified live on the *regenerate* path (row 32) but has **never been observed surviving a live weekly auto-adapt**. **Check:** save a workout on profile 4 on/after that timestamp (`maybeAdaptAllRoadmaps` is fire-and-forget on `POST /api/workouts`, >7d stale only), then confirm the bench goal's **`roadmap.arc_origin` is still `2026-07-26`** and `arc_state` is still present, **while** `roadmap.version` incremented and `adaptation_log` gained a `weekly` entry — i.e. an adapt genuinely ran and the arc survived it. **⚠ Do NOT use the wrist goal** — its regenerate reset `last_adapted_at`, pushing it to 2026-08-04. |
> | **2** | **Shimmy's sandbox exploration begins** | Create a fresh profile in the app, add goals through the **normal UI** (roadmap generation is part of what's being tested — never seed goals), link schedule targets to those goals, then seed history whose exercise names match those targets. `SANDBOX.md` has the full PowerShell workflow. **This closes NO ledger row** — it is for exploring behaviour, not for verifying it. |
> | **3** | **Row 28 / item (c) — WAIT, do not build.** | Route (1) chosen by decision: profile 4 accumulates real qualifying sessions on the bench goal until a genuine peak-then-gap forms. No date will be asserted for the lost origin. Closing check unchanged. **Do not synthesise a re-ramp — and do not use the sandbox for it.** |
> | **4** | **Profile-1 migration — ZERO technical blockers remain. Awaiting Shimmy's decision.** | All three blockers are closed: payload (BUG 1, #43), arc-wipe-on-adapt (BUG 3, #42), and flag semantics (BUG 4, #44 — a migrated profile 1 would otherwise have inherited an unclearable banner). **Remaining cost is two things, both real:** one extra trim-ladder rung (`coachingBrief 2343 → 400`) on the heaviest days, and roadmap regeneration churn (it overwrites three roadmaps carrying genuine `adaptation_log` history). **This is a product call, not a task.** |
>
> **What changed this session (2026-07-28, #44):**
> - **BUG 4 FIXED** (`88a737c`) — the one-way flag. Recompute every evaluation + decouple the
>   assessment from the streak gate. **Verified live, both directions in one sweep.** See the BUG 4
>   section above.
> - **#42's regenerate path VERIFIED LIVE** (ledger row 32) — `arc_origin` and `arc_state` survived a
>   real production regenerate; `last_evaluated` being 74.9 min older than `generated_at` is the
>   proof it was carried rather than recomputed.
> - **Regenerate in-flight guard shipped**, and session #43's double-click claim **corrected** —
>   the exposure was narrower than reported.
> - **Honest banner + goal-card copy shipped** (D3).
> - **Sandbox seeder shipped** (`7ed053b`), profiles 1 and 4 denylisted. No profile seeded.
> - **Two new harness traps logged** (§9): a "before" reference must be a **pinned commit, never
>   `HEAD`** (it silently inverted 3 tests the moment the fix landed), and `assert.deepStrictEqual`
>   **fails across vm realms**.
> - **Logged, NOT built:** the **arc-reset product question** (a genuinely unabsorbable goal has no
>   way back except training — that may be correct or too harsh; needs a product decision), the
>   **regenerate-resets-the-weekly-clock** gotcha, and the misleading **"via Coach Chat"**
>   `adaptation_log` summary string.
>
> | Layer | State |
> |---|---|
> | A — honest timelines + capacity + negotiation | shipped, **verified live** |
> | B — arc_state, decay, re-ramp, timeline flex | shipped, **verified live**; persistence fixed #42 — **regenerate path verified live #44 (row 32), WEEKLY path still open (row 33)**; flag semantics fixed #44; 2 items unit-tested only (rows 11, 13) |
> | C — coexistence engine | shipped, **verified live**; row 22 closed #43; 2 items open (rows 19, 20) |
> | D — session depth | shipped, **verified**; item (c) waiting on real data (row 28) |
>
> **Read before starting:** `ROADMAP.md` §6 → "BUG 4" (the flicker finding and why the streak gate
> must never gate the assessment), §6 → "a regenerate RESETS the weekly-adapt clock" (**why the bench
> goal is the test vehicle, not the wrist goal**), the §7 ledger rows 32–35, §7 → "⚠ PER-GOAL vs
> MACRO" (**the easiest thing to get wrong**), and `SANDBOX.md`.
>
> **Carried forward, do NOT chase with synthetic data — and the sandbox does NOT count as data for
> these:** rows 11 (rehab-vs-skill decay contrast), 13 (flex SHORTEN), 19 (handoff at ≥75%), 20
> (derived target through the week preview), 28 (item (c)). **L1–L4 backlog stays parked.**
>
> **Engine v2 remains PAUSED and its strategy REJECTED going forward.**

## Superseded next-session block (session #43) — kept for the record

> **⟶ DISCHARGED in session #44: the #42 REGENERATE path is now VERIFIED LIVE (ledger row 32), BUG 4
> is fixed, and the sandbox seeder shipped. The WEEKLY path (row 33) is what remains. See the current
> block above. Original text follows.**
