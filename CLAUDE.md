...
- **Projected coverage** (measured live against wger's 844 English exercises): ~96% have a description, ~31% have ≥1 image → of our ~805 wger-linked rows, roughly ~770 descriptions / ~250 images. The ~74 non-wger rows (no `wger_id`) get nothing — stay null, degrade gracefully.
- **`exercise-catalog-merge` now UNIONs array data** (2026-07-17): `muscle_groups_primary/secondary`, `equipment`, and `images` are unioned across the pair (was fill-if-empty, which could drop an enriched loser side); `description` stays fill-if-empty (scalar prose can't merge — winner wins, loser fills a gap); `family`/`wger_id` unchanged (winner authoritative / 1:1). Ensures a cleanup merge never drops a freshly-seeded field.
