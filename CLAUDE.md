...
- **Placement order + guaranteed placement** — unmet targets are sorted: `suggested_day`-present first, then `times_per_week` ascending, then category rank (strength/martial_arts `0` < cardio `1` < mind_body/rehab `2` < other `3`) — so a 1×/week strength target claims a day before a 2×/week cardio fills the open slots. Each unmet slot is placed on the **highest-scoring available day even if the score is negative**; only skipped when **zero** unassigned days remain. "Available" = non-anchor day with **max 1 frequency target per day** (rest days are eligible for conversion). Targets are never silently dropped.
