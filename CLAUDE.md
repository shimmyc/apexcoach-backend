...
A 5-agent UI overhaul is underway. **Agent 1/5 (token foundation)** and **Agent 2/5 (Today tab redesign)** are implemented. The work is additive — the legacy `--bg`/`--accent`/`--text` vars and the 10-theme system above still drive the un-migrated tabs (History/Library/Profile) and are untouched. Agents 3–5 migrate the remaining surfaces.
