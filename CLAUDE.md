...
A 5-agent UI overhaul is underway. **Agent 1/5 (token foundation)**, **Agent 2/5 (Today tab)**, and **Agent 3/5 (bottom nav + modals)** are implemented. The work is additive — the legacy `--bg`/`--accent`/`--text` vars and the 10-theme system above still drive the un-migrated tab *content* (History/Library/Profile) and are untouched. Agents 4–5 migrate the remaining surfaces.
