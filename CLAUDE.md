...
- **Placement order + guaranteed placement** — unmet targets are sorted: `suggested_day`-present first, then `times_per_week` ascending, then category rank (strength/martial_arts `0` < cardio `1` < mind_body/rehab `2` < other `3`) — so a 1×/week strength target claims a day before a 2×/week cardio fills the open slots. Each unmet slot is placed on the **highest-scoring available day even if the score is negative**; only skipped when **zero** available days remain. Targets are never silently dropped.
- **Category cap: 1 frequency target per CATEGORY GROUP per day** — a day may hold multiple frequency targets as long as they're different category groups (Upper Body + Yoga ✓; Upper Body + Lower Body ✗ — both strength; two cardio ✗). `freqCatGroup(activity)` is a generic keyword lookup → `strength` (strength/upper/lower/full body/push/pull) | `martial_arts` (mma/bjj/boxing/grappling/martial) | `cardio` (cardio/run/bike/hike/walk/swim) | `mind_body` (yoga/pilates/stretch/meditation/breathwork) | `rehab` (rehab/pt/recovery/mobility), falling back to `inferWorkoutCategoryServer`. "Available" for placement = non-anchor day with no frequency target of THAT group yet (`dayFreqCats[idx]`). **Anchors don't count toward the cap.**
...
- **Collapsible blueprint (client)** — the Fixed Days / Weekly Targets / Daily Add-ons blueprint (`#schedule-grid`) is **collapsed by default**, so the Schedule card's default view is just the 7-day preview. A `#blueprint-toggle` row (sibling between `#week-preview-section` and `#schedule-grid`) shows **"YOUR TRAINING BLUEPRINT ▸ Edit"** (ember) when collapsed; clicking it expands the blueprint **and enters edit mode** (`openBlueprintEdit` → `schedToggleEdit`). The existing **✓ Done** button collapses back + exits edit. `applyBlueprintChrome()` (called at the top of `renderSchedule`) shows `#schedule-grid` only when editing / AI-build / empty-schedule and otherwise hides it behind the toggle; open state mirrors edit mode and persists in `localStorage.ac_schedule_blueprint_open` (restored into `schedEditMode` at boot). Additive only — `schedRender*`/`schedToggleEdit`/`schedPersist` are unchanged. CSS scoped to `#schedule-card .bp-toggle-*`.
