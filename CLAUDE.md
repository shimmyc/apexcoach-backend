...
- **STEP 1 — rule engine (pure JS, no AI, `buildWeekSkeleton`)**: builds the current Mon–Sun week. (a) **anchors** locked from `schedule.anchors[day]`, never moved, `done`-flagged when a completed workout exists that date; (b) **frequency targets** — for each target short of `times_per_week` this week (done counted by coarse `inferWorkoutCategoryServer` category), place each remaining slot on the best open (non-anchor) day, preferring `suggested_day`, else scoring: `+30` not adjacent to a hard day (strength/martial_arts/cardio), `+20` muscle group not hit in 48h, `+10` mid-week (wed/thu), `-20` day already has a target; (c) **addons** attached to every training day; (d) **rest** fills the remainder; (e) **muscle-recovery map** from the last 7d of actual (done) workouts via `inferWorkoutCategoryServer` + a finer `previewMuscleGroup` upper/lower/full_body keyword split (48h windows for strength/full_body; martial_arts/cardio/mind_body/rehab no block) feeds the `+20` scoring. Output: 7 objects `{ dayKey, date, dayLabel, planned:[{activity,type:anchor|frequency_target|addon|rest,duration,category}], done, actual_workout, recovery_notes }`.
- **STEP 2 — Haiku** (`enrichWeekPreviewWithCoaching`, S&C-coach persona): given the skeleton + profile (ai_prompt_context 600c) + last 7d workouts + today's readiness + micro-goal titles, returns `{ week_note, days:[{dayKey, coaching_note ≤12 words}] }`, merged onto the skeleton. **Non-fatal + 6s `Promise.race` cap** — returns the bare skeleton on timeout/failure. Response: `{ success, week:[7], week_note, generated_at }`. Generic: zero hardcoded user/schedule assumptions.
- **Client** (`public/index.html`): `loadWeekPreview()` (cache-first via `localStorage.ac_schedule_preview` = `{date, profileId, data}`; same-day+same-profile → render from cache, else POST with `schedActiveSchedule()` + `readiness.score`), `renderWeekPreview(data)` (compact 7-row list — 3-char day label colored by past/today/future, activity with `+ add-ons`/`+N more` suffix, muted coaching note, `✓`/`•` status, today gets a 3px ember left border, past rows `opacity:.7`, week_note truncated ~60c with `title=`, a muted Refresh link → `wpRefresh()`), and `invalidateSchedulePreview()` (clears cache; re-fetches only if the preview is on screen). CSS scoped to `#week-preview-section`.
